...
- **`ProjectionRebuildReport`** — the metadata-only NFR57 **validation-evidence artifact** (tenant ref, dataset ref, start/end, measured rebuild duration, `DurationWithinTarget`, verdict, source/governed/WORM cardinalities, bounded deviations, first-diverging locator, both projection schema stamps, complete digest lists, fingerprint algorithm version, two canonical fingerprints, correlation id, reason code). The manifest independently replays the ordinal length-framed SHA-256 fingerprints and binds schema/count/verdict/reason semantics. `Unmeasurable(...)` retains empty snapshot evidence rather than fabricating equivalence.
...
**Independent governed/WORM rebuild update.** Seed and rebuild inputs are independently loaded and appended. The baseline directly seeds its expected persisted views without using the rebuild reconstructor or handlers; the rebuild groups WORM envelopes by `(resource id, correlation id)`, reconstructs each operation through `AuditOperationReconstructor`, and applies it through `GovernedOperationProjectionHandler` in deterministic chain order. Complete source-plus-governed digest lists and canonical fingerprints are retained and independently replayed by the gate. This retires `RV-REBUILD-WORM` for the bounded sandbox path. The historical hosted run `33066358280` (commit `17aa94d`) predates this evidence contract and remains historical only; a new hosted bundle is still needed to authenticate the strengthened path. `RV-DURABLE-WORM`, `RV-PROVIDER-SCALE`, hosted-evidence freshness, and `RV-MEASURABLE-CEILING` remain, so NFR57 is not discharged.
