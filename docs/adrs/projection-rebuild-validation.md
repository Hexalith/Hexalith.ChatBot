...
**Story 12.15 implementation update (partial retirement).** The Tier-3 driver reads a separately seeded persisted baseline, writes and reads back a distinct DAPR partition through the production projection-store abstractions, and performs ETag cleanup. **Retired:** the "no real store round-trip" half of the deferral — that part is genuine. **Not retired: the NFR57 equivalence claim itself.** No projection translator or handler is exercised (`MailboxIntakeProjectionTranslator` / `AssociationProjectionHandler` are never invoked); the driver writes an identity copy of an already-projected view; and the live lane supplies the same source-record instance to both the baseline seeding and the driver, so `divergent` is unreachable and `equivalent` is a tautology. No measured rebuild duration is published — the earlier local figures rest on that comparison, and the diagnostic bundle they came from cannot be replayed through the shipped evidence gate. The lane's 180-second restoration ceiling also sits far below the 4-hour NFR57 target, so no run from it can demonstrate a duration miss.

The serialized CI/release gate invokes `RunAllAsync` on the scheduled, manually dispatched and release lanes only — ordinary `push`/`pull_request` CI runs no recovery validation. Product DI deliberately remains inert. Durable production WORM and production-volume evidence remain `RV-DURABLE-WORM` / `RV-PROVIDER-SCALE`, and the ceiling gap is `RV-MEASURABLE-CEILING`.
