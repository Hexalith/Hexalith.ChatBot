...
- **`IProjectionRebuildDriver`** — the seam the coordinator consumes for the snapshots + measured duration. The **live rebuild runtime is M2-deferred**; the inert `DeferredProjectionRebuildDriver` throws `NotSupportedException`.
...
**Deferred** (documented seams, exactly like Story 9.4's deferred replay driver, 9.11's deferred fault-injection runtime, and 9.1/9.2/9.4/9.5/9.11's deferred periodic scheduler):

1. The **live rebuild driver** that actually replays a tenant's immutable source records + WORM history into a fresh projection store against a deployed AKS/Aspire environment — modeled behind `IProjectionRebuildDriver` (inert default throws `NotSupportedException`).
2. The **periodic scheduler / release-gate wiring** — no always-on `BackgroundService`; a scheduler/gate need only call `RunAllAsync` on its cadence and supply the real driver + the test tenant + baseline dataset refs.

**The deferral never reads as "rebuild/recovery is unproven."** The equivalence semantics, the duration-vs-target check, and the fail-closed evidence path are all real and tested against the rebuild-driver seam.
