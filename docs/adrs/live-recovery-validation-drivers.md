...
| `projection-rebuild` | Fresh DAPR validation partition selected by an independently loaded versioned dataset; the driver reads immutable `ProjectConversationSourceEmailView` metadata plus a separately appended `IWormAuditStore.EnumerateChain` only. | A distinct seed dataset and WORM store directly materialize the persisted baseline oracle; zero source or audit coverage, unsafe resource ids, and any of the six missing dataset categories are rejected. | Source-email records replay through the real `AssociationProjectionHandler`. WORM envelopes are grouped by governed resource and correlation, reconstructed through `AuditOperationReconstructor`, and replayed through `GovernedOperationProjectionHandler` in chain order. The complete source-plus-governed snapshot is read back, retained as bounded metadata-only digests with length-framed canonical fingerprints, and ETag cleanup erases every reachable constrained write. **Neither source-email nor governed equivalence is an identity tautology.** | Only source-email and WORM records are projection inputs. The other four parsed categories establish configured-corpus provenance but are never materialized, seeded or compared. The manifest reports `configuredDatasetVolume: 6` and the actual source-plus-unique-governed-resource count in `datasetVolume`; it separately records WORM record, operation, and governed-resource cardinalities. WORM remains process-local and scale is non-production. |
...
The production `IReadModelStore` abstraction does not enumerate partition keys. Completeness is therefore bounded to the driver's closed write set: one source key per validated unique intake id and one governed key per validated unique WORM resource id. The driver rejects unsafe/duplicate logical ids, asserts the exact collision-free key cardinality before writing, reads every reachable key back into the snapshot, and ETag-erases that same set. It does not claim to discover unrelated pre-existing keys outside this fresh constrained partition.
...
`RV-REBUILD-WORM` is retired for the bounded Tier-3 sandbox path: governed history now rebuilds through the production reconstructor and projection handler, and independent retained fingerprints expose same-resource structural mutations. This retirement does not close durable-WORM, provider-scale, hosted-evidence, or measurable-ceiling limitations.
...
Even a passing sandbox run cannot close `RV-EXT-M365`, `RV-DURABLE-WORM`, `RV-PROD-CONTROL`, `RV-PROVIDER-SCALE`, `RV-MEASURABLE-CEILING`, or `RV-EVIDENCE-KINDS`. Tightening or loosening still requires the PRD decision, this ADR, and `RecoveryTargets` to change together with retained evidence, rationale, and approval.
