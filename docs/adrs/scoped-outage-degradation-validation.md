...
- **`IScopedOutageInjectionDriver`** — the seam the coordinator consumes to inject the outage + run the assertions. The **live fault-injection runtime is M2-deferred**; the inert `DeferredScopedOutageInjectionDriver` throws `NotSupportedException`.
...
**Deferred** (documented seams, exactly like Story 9.4's deferred replay driver, 9.11's deferred fault-injection runtime, and 9.12's deferred rebuild driver):

1. The **live fault-injection runtime** that actually downs a real Graph/identity/AI/command/audit/attachment dependency against a deployed AKS/Aspire environment and measures the real degradation scope + assertions — modeled behind `IScopedOutageInjectionDriver` (inert default throws `NotSupportedException`).
2. The **periodic scheduler / release-gate wiring** — no always-on `BackgroundService`; a scheduler/gate need only call `RunAllScenariosAsync` on its cadence and supply the real driver + the test tenant.

**The deferral never reads as "scoped degradation is unproven/unmeasured."** The containment semantics, the three NFR59 assertions, the recovery-resume checks, the 5-min scope-recording check, and the fail-closed evidence path are all real and tested against the driver seam.
