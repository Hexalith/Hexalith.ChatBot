...
#### Code review 2026-09-13 — Story 1.1c re-verification (range `8c3dd15~1..9567f43`)

- [ ] [Review][Patch] Enumerate the EventStore caller's invoked route set in the production DAPR ACL [src/Hexalith.ChatBot.AppHost/DaprComponents/accesscontrol.yaml:27] — DECIDED 2026-09-13: replace the single `/process` allow with explicit POST-scoped allows for `/process`, `/replay-state`, `/query`, `/project`, `/project/v2`, the six `/project/rebuild/*` routes and `/admin/operational-index-metadata`, and restore a rationale comment. Context: The narrowing from `/**` to `/process` also deleted the comment justifying the wildcard, without rebutting it. ChatBot's Server calls `UseEventStoreDomainService()` and registers `IDomainProjectionHandler` + `IDomainQueryHandler`, so its DAPR-invoked surface is `/process`, `/replay-state`, `/query`, `/project`, `/project/v2`, six `/project/rebuild/*` routes and `/admin/operational-index-metadata`; `DaprAggregateStateReconstructor` invokes `replay-state` cross-app from appId `eventstore`. Under the mTLS-on posture this file describes, everything but `/process` is now denied. Nothing can catch it: the local topology loads `accesscontrol.local.yaml`, and no deployment manifest in this repo consumes `accesscontrol.yaml` at all. Choose either restoring the POST-scoped wildcard with its rationale, or enumerating the exact invoked route set.
- [ ] [Review][Patch] Add a dead-letter subscriber for the projection dead-letter topic [src/Hexalith.ChatBot.Server/Gateway/ChatBotCompatibilityEndpointExtensions.cs:125] — DECIDED 2026-09-13: map a subscribed dead-letter endpoint so poison messages surface. Context: Seven projection subscriptions now route poison messages to `deadletter.tenant-alpha.chatbot.events` and nothing subscribes to it, drains it, or alerts on it. Failures move from "retried" to "silently accumulating in Redis". Choose a dead-letter subscriber/alert, or record accepting silent accumulation for M0 with a named reopen point.
- [ ] [Review][Patch] Record an explicit non-production seed policy for service-client grants [src/Hexalith.ChatBot.AppHost/Program.cs:189] — DECIDED 2026-09-13: document these as local-development seed credentials only, keep the pre-expiry gate as the guardrail, and keep production provisioning out of the AppHost (consistent with `IsPublishable=false`). Context: The prior finding asked for a policy "so every client does not fail simultaneously"; a computed constant replaced a literal constant and all six clients still share one `expiresAt`. Nothing re-issues or rotates. Choose staggered per-client expiries, a rotation path outside the AppHost, or an explicit non-production seed policy.
- [ ] [Review][Patch] Warn when the pre-expiry gate runs under `KeycloakPersistent=true` [src/Hexalith.ChatBot.AppHost/Program.cs:238] — DECIDED 2026-09-13: keep the gate, but emit a clear warning that the validated expiry may not be the one Keycloak is serving, pointing at the documented `docker rm -f` re-import step. Context: EventStore's own wiring documents that a reused Keycloak container does NOT re-import the realm. In persistent mode the AppHost computes and validates a fresh expiry that Keycloak never receives, so the gate passes against a value that was never applied. Choose hard-fail, warn, or forced re-import.
- [ ] [Review][Patch] Cover the substituted grant-expiry value with a test that reads it through the consumer — `DateTimeOffset.ToString("O")` rendered `+00:00`, which `AuditMetadata.IsSafeToken` rejects (`+` is outside its charset), so `ClaimsServiceClientGrantResolver` would have denied every service client with `service_client_grant_missing`. HEAD fixed the rendering (`expiresAt.UtcDateTime`), but the gap that let it ship is open: every resolver test hand-writes `...Z` literals, the AppHost test asserts only the placeholder, and the Tier-3 lane mints a *user* token (`grant_type=password`), so it can never exercise a service client. Replace the source-text guard with a test that reads the substituted claim back out of the generated realm and asserts `IsSafeStableIdentifier` accepts it [src/Hexalith.ChatBot.AppHost/Program.cs:252]
- [ ] [Review][Patch] Assert the topology-acceptance TRX actually ran a test — `dotnet test --filter FullyQualifiedName=...` exits 0 when the filter matches nothing (confirmed against VSTest 18.7.0 in this repo), so a rename or move of the topology test turns the required gate green while proving nothing. `release.yml` gates `semantic-release` on this job. The repo already has the guard for `story132-topology-acceptance.trx`; `topology-acceptance.trx` has none [.github/workflows/ci.yml:500]
- [ ] [Review][Patch] Restore exhaustive deny-by-default coverage in the ACL conformance test — the rewrite replaced a whole-file `ShouldNotContain("defaultAction: allow")` with structural checks on two named policies and never asserts the policy list is complete, so a third policy carrying `defaultAction: allow` now passes [tests/Hexalith.ChatBot.Conformance.Tests/DaprAccessControlConformanceTests.cs:26]
- [ ] [Review][Patch] Guard dead-letter routing on all seven subscribers and stop duplicating the topic literal — `AppHostShouldWireSubscriberDeadLetterRoutingIntoDaprDiscovery` reads only `GovernedOperationProjectionEndpoints.cs`, so six subscribers can lose `DeadLetterTopic` silently. `DeadLetterTopicName` also remains referenced only by its declaration and two test assertions — the "unused constant" the prior finding named is still unused — while `ChatBotCompatibilityEndpointExtensions.cs:125` re-hardcodes the same string as an unshared fallback [tests/Hexalith.ChatBot.AppHost.Tests/AppHostTopologyTests.cs:321]
- [ ] [Review][Patch] Assert every required resource's state and endpoint, not just the chatbot's — `Snapshot.State.ShouldNotBeNull()` never checks the state text, and `endpoints.ShouldNotBeEmpty` is gated to `ChatBotResourceName`; `security`, `chatbot-ui` and `eventstore-admin-ui` remain recorded-but-unasserted at HEAD [tests/Hexalith.ChatBot.IntegrationTests/TrivialGovernedCommandAspireE2eTests.cs:196]
- [ ] [Review][Patch] Make the fail-closed proof non-vacuous and order it after the sidecar guard — `ShouldNotBe(HttpStatusCode.OK)` is satisfied by 500, 503 or a sidecar timeout, so the "no durable state" proof is greenest when the read path is broken; assert `NotFound`. The probe also issues sidecar-backed reads at line 174, above the `WaitForChatBotDaprSidecarAsync` call at line 186 whose own comment says the state-store path must be verified BEFORE submitting [tests/Hexalith.ChatBot.IntegrationTests/TrivialGovernedCommandAspireE2eTests.cs:174]
- [ ] [Review][Patch] Bound the topology wait to the job budget — seven resources each get `WaitAsync(TimeSpan.FromMinutes(5))` sequentially (35 minutes) inside a job with `timeout-minutes: 30` that at HEAD also runs a browser E2E. A partial-startup hang is killed by the runner, skipping the `if: always()` artifact upload, so the lane loses the evidence it exists to produce [tests/Hexalith.ChatBot.IntegrationTests/TrivialGovernedCommandAspireE2eTests.cs:176]
- [ ] [Review][Patch] Compare derived fields instead of raw JSON in the stability assertion — `current.GetRawText().ShouldBe(expectedBody)` flakes on any volatile field or property reorder in a now-required release gate; `DerivedRecordShape` already exists in this file for exactly this comparison [tests/Hexalith.ChatBot.IntegrationTests/TrivialGovernedCommandAspireE2eTests.cs]
- [ ] [Review][Patch] Clean up `IsTransientStatusCode` — `(int)statusCode >= 500` already subsumes the enumerated 502/503/504 (dead branches) while still retrying genuinely permanent 501/505 for the full window, the opposite of the fail-fast the prior finding asked for. Unverified: whether the 404/401 warm-up window is reachable (Keycloak is wired with `--import-realm`, which completes during bootstrap, and the lane waits for `security` healthy first) — a cold-start run logging the first non-200 status would settle it [tests/Hexalith.ChatBot.IntegrationTests/TrivialGovernedCommandAspireE2eTests.cs:2584]
- [ ] [Review][Patch] Stop hand-maintaining `expectedServiceGrantCount` — it has already drifted once (6 at this commit, 7 at HEAD): adding a realm client makes the AppHost refuse to start until the literal is edited, with a message that names a count rather than the missing client [src/Hexalith.ChatBot.AppHost/Program.cs:189]
- [ ] [Review][Patch] Condition the `HexalithCommonsRoot` forwarding on a non-empty value — `AdditionalProperties` pins the property as a *global* property, which referenced projects' own `Condition="'$(HexalithCommonsRoot)' == ''"` fallbacks cannot override, turning an undetected root into a hard break instead of a fallback. Inert today only because Commons is present [src/Hexalith.ChatBot.AppHost/Hexalith.ChatBot.AppHost.csproj:38]
- [ ] [Review][Patch] Give `DerivedRecordShape`'s `GetProperty` calls a message — the `TryGetProperty` → `GetProperty` tightening is right, but a missing field now throws a bare `KeyNotFoundException` naming neither the field nor the origin, inside a required gate [tests/Hexalith.ChatBot.IntegrationTests/TrivialGovernedCommandAspireE2eTests.cs]
- [x] [Review][Defer] Realm containing plaintext credentials written to a predictable, world-readable temp path [src/Hexalith.ChatBot.AppHost/Program.cs:193] — deferred: already resolved after this range. HEAD uses `Directory.CreateTempSubdirectory`, `UnixCreateMode = UserRead|UserWrite`, an owner marker and a stale-directory sweep, guarded by `RecoveryValidationTopologyContractTests`.
- [x] [Review][Defer] `keyPrefix` isolation is partly inert and asserted by counting substrings [src/Hexalith.ChatBot.AppHost/Aspire/ChatBotAspireModule.cs:209] — deferred: the shared `statestore` keeping `none` is defensible (one component shared by eventstore+tenants; prefixing would not separate them), and the AppHost is local-only. Unverified: whether `keyPrefix: name` does anything on `chatbot-workflow-statestore`, where `actorStateStore: true` means DAPR composes keys as `appid||type||id||key`. A Redis key dump from a live run would settle it.
- [x] [Review][Defer] Workflow hygiene gaps in the new topology jobs [.github/workflows/ci.yml] — deferred: mostly resolved after this range. HEAD pins `dapr init --runtime-version 1.18.0` and carries `permissions: contents: read`. The remaining verbatim duplication of the job between `ci.yml` and `release.yml` is cosmetic.
- [x] [Review][Defer] `dotnet-version: 10.0.302` contradicts `global.json` 10.0.400 [.github/workflows/ci.yml:29] — deferred: pre-existing and repo-wide (all six `setup-dotnet` steps in `ci.yml` and every one in `release.yml`); the new jobs followed the existing convention rather than introducing the drift.

##### Rejected

- `false`/spec-edit — Decision-record erasure, missing File List entries, and the `review`→`done` flip: the three `[Review][Decision]` items were rewritten in place as `[Review][Patch]` and self-ticked, losing the record of what was chosen and why. Rejected because the fix is to edit the story artifact under review. (The substantive half — that the cited green lane cannot exercise service-client identity — is carried by the grant-expiry patch above.)
- `low` — `WithExternalHttpEndpoints()` widening the local topology's exposure to satisfy an endpoint assertion: harm needs a hostile LAN on a local-only AppHost, and the fix is a dev-only gate rather than a direct correction.
- `low` — `EnableKeycloak=false` throwing in `PrepareKeycloakRealmImport`, and `RequiredTopologyResources` hardcoding `"security"`: nothing in this repo sets that key, and the fix adds branches.
- `maybe-false` — Fluxor / CommunityToolkit Dapr bumps filed under "so the AppHost restores and starts": the stated justification looks wrong (the AppHost does not load a Blazor state library), but a CPM catalog bump can legitimately be required for restore coherence, and no bad outcome was demonstrated. Would be `low` if true.
- `maybe-false` — `GetTenantDeadLetterTopic` not validating tenant-id characters: only whitespace/null is guarded, but the sole caller passes the literal `"tenant-alpha"` and M0 is single-tenant. Would be `low` if true.
