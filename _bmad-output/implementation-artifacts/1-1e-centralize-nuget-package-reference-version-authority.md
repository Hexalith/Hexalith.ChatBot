...
5. **Catalog-first rollout evidence is complete.** Given one recorded ChatBot baseline and its exact Builds/consumer gitlink matrix, when completion evidence is recorded, then the Builds authority and every consumer pinned by ChatBot pass their relevant restore, canonical build, focused test, package/consumer, and resolved-graph lanes from isolated standalone checkouts at those exact commits. Each standalone checkout initializes only dependencies declared by its own root `.gitmodules`, using explicit non-recursive pathspecs; dependencies of those initialized checkouts remain uninitialized, while standalone Timesheets may initialize its own root-declared `Hexalith.Works`. No dependency checkout beneath the ChatBot umbrella is initialized, no `.gitmodules` entry or consumer gitlink is changed, and no dependency project is removed from a consumer `.slnx` to make validation pass. Only after every standalone lane is green does the unchanged ChatBot superproject pass restore/build, package-authority architecture tests, affected UI/architecture suites, and relevant integration lanes at the same recorded baseline without a local override. Evidence names the repository, pinned commit, root-dependency commits, exact command, exit result, warning/error and test totals, effective-version changes, clean-tree result, and any blocker. [Source: _bmad-output/planning-artifacts/epics.md#Story-1.1e-Centralize-NuGet-package-reference-version-authority; Source: _bmad-output/planning-artifacts/sprint-change-proposal-2026-07-18.md#Implementation-Sequence-and-Success-Criteria; Source: _bmad-output/planning-artifacts/sprint-change-proposal-2026-07-20.md#Standalone-Checkout-Evidence-Contract]
...
- [ ] Produce independent consumer and umbrella completion evidence (AC: 3-5)
  - [ ] Freeze one ChatBot root commit and record its exact Builds/consumer gitlink matrix. If a pinned gitlink changes before the evidence set is complete, freeze a new root baseline, rerun every changed row, and verify every reused row still has the identical pinned SHA; never attribute evidence to a different commit.
  - [ ] For Builds and each pinned consumer, create a disposable isolated standalone checkout outside the ChatBot worktree, detach it at the recorded commit, and prove its pre-validation tree is clean and `git rev-parse --show-superproject-working-tree` is empty.
  - [ ] Inspect that standalone root's tracked guidance and root `.gitmodules`, then initialize only its root-declared dependency paths with an explicit non-recursive command. Record every initialized path and gitlink SHA and prove submodules declared by those dependency checkouts remain uninitialized. Standalone Timesheets may initialize its own root-declared `Hexalith.Works` at its tracked gitlink.
  - [ ] In each standalone checkout, run its canonical `.slnx` restore and build using the repository's documented serialization/configuration; run its documented focused tests rather than assuming every Hexalith repository shares one test runner convention.
  ...
  - [ ] Before the final ChatBot stage resumes, publish the complete standalone evidence dossier required by the resumption gate below; historical umbrella-nested evidence and partial-checkout workarounds do not qualify.
  - [ ] After every standalone lane is green at one baseline, run ChatBot `dotnet restore Hexalith.ChatBot.slnx`, canonical Release build, package-authority scans, affected focused test projects, and relevant integration lanes without initializing nested submodules.
...
- Root-declared submodules only, where root means the isolated repository checkout currently being validated; no recursive or remote update. The ChatBot umbrella may initialize only ChatBot root entries and never entries owned by those dependency checkouts. Changes inside a submodule belong to that repository and require separate validation/commit intent.
...
### Standalone-Checkout Validation and Resumption Gate

The observed evidence baseline for this correction is ChatBot `bd652e3c61ebfa0202f6a1fdb696759637a21bca`. Before execution, freeze the then-current ChatBot root and verify its complete gitlink matrix. A later documentation-only ChatBot commit may be used when every listed gitlink is unchanged. If a gitlink changes, rerun that changed row and verify every reused row still has the identical pinned SHA; evidence may never be attributed to a commit other than the one actually validated.

| Role | Standalone repository | Commit pinned by ChatBot `bd652e3` |
| --- | --- | --- |
| Package authority | `Hexalith.Builds` | `e4ae82df6cfcc6511a32fc2ce100070d7924f119` |
| Consumer | `Hexalith.EventStore` | `afcc167e0c539b09ecad978a58da2f756123f34e` |
| Consumer | `Hexalith.Tenants` | `f03b474d836a3465e311c48e90c75ae1e755ef45` |
| Consumer | `Hexalith.FrontComposer` | `550cb0602d506d9fd008a8c09f2cca6b328ec1e3` |
| Consumer | `Hexalith.Folders` | `cfe830b410bce6e04308ea67c3492eca6bc8bdfd` |
| Consumer | `Hexalith.Conversations` | `6e8cf8a6605142808d1afede3f2d0e29541f0e08` |
| Consumer | `Hexalith.Projects` | `fca2bfc050f7a581a467aaa3921e2aa61f249e72` |
| Consumer | `Hexalith.Parties` | `b316dab5cf27d9f80a662b5b3cd6c5e2569adfd7` |
| Consumer | `Hexalith.Memories` | `f474db156c372aad4ab243c13a669c35d78b49e6` |
| Consumer | `Hexalith.Commons` | `ea1fc4551dcaf8ee63fd562d77dfe0f18c57a94c` |
| Consumer | `Hexalith.Timesheets` | `441f02509cfd43c888e2d4317a167b41657208b4` |
| Consumer | `Hexalith.PolymorphicSerializations` | `a5dd24f5e66324d18241de7d5521ee124eab4877` |

Before Story 1.1e resumes its final umbrella stage, one evidence dossier must contain all of the following:

1. **Identity and isolation proof for every row:** repository URL, ChatBot gitlink path, ChatBot baseline SHA, expected pinned SHA, standalone `HEAD`, an empty superproject-working-tree result, and clean `git status --short` output before validation.
2. **Root-dependency proof:** the standalone repository's root `.gitmodules` path list, each direct gitlink SHA, the exact non-recursive initialization command with explicit pathspecs, and post-init status showing only those direct paths initialized. Every submodule declared by an initialized dependency remains uninitialized. For Timesheets, this explicitly permits its direct `Hexalith.Works` gitlink `f2259daab922096113262fc9e0a5588182918e0a` and direct `references/Hexalith.Builds` gitlink `f0750ca703cc3ada6eb25050cb6b287e83ce3938`; neither Works nor Builds may initialize its own dependencies.
3. **Canonical repository result:** exact restore and Release build commands, exit codes, zero-warning/zero-error totals where the owning repository requires that gate, and the owning repository's documented focused test commands with passed/failed/skipped totals. A blocked or unrun required lane is not green evidence.
4. **Package-governance result:** applicable catalog, consumer-authority, exception, family-alignment, package/pack, and resolved-graph commands and results; the effective package values must match the approved matrix with no consumer-local version escape hatch or NU1109 downgrade.
5. **Integrity proof:** no ChatBot or consumer `.gitmodules` edit, no consumer `.slnx` project removal, no gitlink change, no generated or source workaround created only for validation, and clean standalone `git status --short` plus `git diff --check` after validation.
6. **Gate summary:** a row-by-row PASS table for Builds and all eleven standalone consumers, signed against one ChatBot baseline. Only that complete green table authorizes the final unchanged-umbrella rerun; historical evidence obtained by initializing nested checkouts inside ChatBot is explicitly non-canonical.

ChatBot is the twelfth .NET consumer and is validated as the final umbrella lane at the recorded root commit after the authority and eleven standalone consumer rows pass. Story and sprint status remain `in-progress` until both the standalone dossier and renewed ChatBot umbrella evidence are complete.
...
- Each consumer: from its isolated standalone checkout at the ChatBot-pinned commit, initialize only that checkout root's declared dependencies non-recursively, restore its canonical `.slnx`, build with its repository-required configuration/serialization, run focused compatibility tests and package-only Release validation where configured, and capture resolved graph evidence for changed canonical versions.
...
- Root-only non-recursive submodule initialization was added after review because `submodules: false` alone could not restore dependencies. Interpret root as the isolated checkout under validation: ChatBot root dependencies only in ChatBot; consumer root dependencies only in the standalone consumer. Never replace the explicit path list with recursive checkout.
...
- Changing ChatBot or consumer `.gitmodules`, adding an umbrella-level `Hexalith.Works`, or removing dependency projects from a consumer `.slnx` to manufacture a green lane.
