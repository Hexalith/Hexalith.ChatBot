...
- Follow the domain-module solution shape with `.slnx`, .NET 10/C# 14, warnings as errors, and individually executed xUnit v3 test projects. In ChatBot, cross-repository development uses only ChatBot root-declared sibling submodules, initialized non-recursively; no dependency-owned submodule is initialized beneath them. Independent consumer validation uses an isolated standalone checkout at the ChatBot-pinned gitlink, where only that standalone root's declared dependencies may be initialized non-recursively.
