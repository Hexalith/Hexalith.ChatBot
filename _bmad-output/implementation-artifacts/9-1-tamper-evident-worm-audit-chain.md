...
- **[LOW · noted]** The AC2 five-minute SLA assertion is satisfied by-construction (synchronous detection→emit under a `FixedClock`), so the test is effectively tautological. This was originally accepted because the periodic scheduler was an explicitly-documented inert-control-floor deferral; **that deferral was retired by Story 12.14** (see "Runtime activation confirmed" below), so the by-construction assertion is now the weaker half of a real scheduled path rather than the only evidence.
