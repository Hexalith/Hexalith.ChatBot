...
- [x] [AI-Review][CRITICAL] **RESOLVED via pivot to a ChatBot-owned hub (2026-06-21)**, after the live Tier-3 run exposed that the reuse relay could not deliver in the real topology. **Live finding:** the ChatBot Aspire topology defines a SINGLE pub/sub component `chatbot-pubsub` (no `pubsub`); the prior `DaprProjectConversationChangePublisher` hardcoded `"pubsub"` (fails-open silently); EventStore's `ProjectionNotificationController` subscribes on the compile-time const `"pubsub"` (can't receive on `chatbot-pubsub` w/o an EventStore submodule edit); and `AddEventStoreSignalR` was OFF. **Pivot (Jerome's call):** ChatBot.Server now hosts its OWN metadata-only SignalR hub `ChatBotProjectConversationHub` (tenant-grouped, mapped at `/hubs/chatbot/project-conversation-changes` when enabled) and broadcasts via `SignalRProjectConversationChangePublisher` (IHubContext → tenant group, fail-open); the UI `ProjectConversationStreamingSubscriber` opens a `HubConnection` to the ChatBot server, `JoinTenant`, reacts to `ProjectConversationChanged`, rejoins on reconnect → dispatches the signal action → effect synthesizes the metadata-only nudge → typed re-query. This sidesteps every cross-host problem AND is **directly verifiable in one host (no DAPR):** `ChatBotProjectConversationHubE2ETests` drives a REAL SignalR HubConnection over a WebApplicationFactory — joins the tenant group, projects a low-risk AI outcome (producer sets progress), and asserts the client receives the tenant-scoped signal while a DIFFERENT tenant's group does NOT. The live Tier-3 durable spine stays green (22/0) with the pivot. **Remaining honest caveats:** (a) with today's disabled/synchronous AI provider the in-flight `rendering` window is zero-width (the transport delivers, but there is no long-lived in-flight state to show until a real async streaming provider, M2+); (b) the hub allows unauthenticated joins in dev/test and fail-closes cross-tenant when authenticated — wiring the UI `HubConnection` access token for a JWT-on deployment is a remaining prod concern. [src/Hexalith.ChatBot.Server/Projections/ChatBotProjectConversationHub.cs] — Superseded dev-pass (reuse) detail follows:
- [x] [AI-Review][CRITICAL] **(SUPERSEDED — original reuse design; the producer + MEDIUM fix + UI effect machinery below still stand, but the DAPR-relay transport was replaced by the ChatBot-owned hub above.)** Original note, after an explicit two-decision design pass with Jerome (producer approach + transport carrier). **Producer:** `LowRiskAiOutcomeProjectionTranslator` projects the REAL governed low-risk AI-assistance execution lifecycle — the `executing` started event becomes a non-terminal `rendering` `AiResponseProgress`; `succeeded`/`failed`/`routed-to-approval` become server-verified terminal `completed`/`failed`/`unavailable` — so `BuildAiResponseProgress()` returns non-null progress (`ActiveStreamingProgress` non-null during a real generation, Stop control can enable). No synthetic generation. **Transport (reuse, signal-only):** on an AI-progress upsert, `InMemoryProjectConversationProjectionStore` publishes a metadata-only EventStore `ProjectionChangedNotification("project-conversation", tenantId)` via DAPR pub/sub (`DaprProjectConversationChangePublisher`, gated by `ChatBot:ProjectionChangeNotifications:Enabled`, no-op by default); the running EventStore host relays it on its `ProjectionChangedHub`; `ChatBotProjectConversationWorkspace` (via `ProjectConversationStreamingSubscriber`, reusing the FrontComposer `IProjectionSubscription`/`IProjectionChangeNotifierWithTenant` the UI already registers) joins the tenant-scoped group and dispatches `ProjectConversationProjectionSignalReceivedAction`. The effect synthesizes a forward-looking metadata-only nudge for the current conversation and dispatches `ProjectConversationAiResponseNudgeReceivedAction` → typed re-query. The nudge/reducer/effect machinery now runs at runtime. **Caveats (documented, not gaps in this wiring):** (a) cross-process delivery (publish → EventStore relay → UI receive) is only exercisable on the live DAPR/Aspire + EventStore topology, so in-sandbox verification is per-seam (recording-publisher unit test, effect unit tests, source-scan of the subscription) + the live flag; (b) with today's disabled/synchronous AI provider the in-flight `rendering` window is zero-width (started+completed are atomic), so the *visible* Stop-enabled window materializes when a real async streaming provider lands (M2+) — the same wiring then shows the in-flight progress. [src/Hexalith.ChatBot.UI/State/ProjectConversation/ProjectConversationEffects.cs]
- [x] [AI-Review][MEDIUM] **Resolved (2026-06-21, dev pass).** The nudge re-query effect and the nudge reducer no longer disagree. `HandleAiResponseNudgeAsync` now injects `IState<ProjectConversationState>` and re-queries **iff the reducer (which Fluxor runs first) accepted this exact nudge** (`ReferenceEquals(_state.Value.LastAcceptedAiResponseNudge, action.Nudge)`), so the single fail-closed acceptance gate (cross-project + metadata-only + stale/out-of-order) lives in `IsAcceptableNudge` and the effect can never re-query a nudge the reducer rejected. Verified by `NudgeEffectShouldRequeryIffReducerAcceptedTheNudgeSoEffectAndReducerAgree` (drives the real reducer then the effect). The architecture test's `LoadProjectConversationAction(action.Nudge.ProjectId)` pin is preserved. [src/Hexalith.ChatBot.UI/State/ProjectConversation/ProjectConversationEffects.cs]
- [ ] [AI-Review][LOW] (carried, unchanged — intentional foundation behavior) Server-side verify that the targeted generation/response actually exists and is in-flight before emitting `AiResponseGenerationCancellationRequested`; the aggregate is keyed by `CancellationId`, so it validates metadata only. The producer added in this pass projects the *low-risk execution* lifecycle into read-model progress but does NOT introduce a server-side generation-session aggregate the cancellation could read to confirm in-flight state (aggregates do not read projections); a meaningful in-flight guard still needs that lifecycle, which arrives with a real async streaming provider (M2+). Left deferred deliberately, consistent with Passes 1–5. [src/Hexalith.ChatBot.Server/Operations/GovernedOperationAggregate.cs:1648]
...
### Review Findings (2026-06-21 — Opus 4.8 multi-agent adversarial review)

Scope: full story diff `0218429..HEAD` (the transport pivot is the new, never-before-reviewed surface). Three independent layers (Blind Hunter / Edge Case Hunter / Acceptance Auditor) → 20 raw → 16 deduped → adversarial verify: **5 confirmed, 11 dismissed (false positives, incl. the 3 documented recurring FPs + the zero-width-window M2+ caveat).** The previously-CRITICAL AC1 gap is independently re-confirmed RESOLVED: the producer (`LowRiskAiOutcomeProjectionTranslator`) is genuinely wired at runtime (`AiOutcomeProjectionHandler.cs:35`), and `ChatBotProjectConversationHubE2ETests` is a real HubConnection→broadcast→receive E2E (not vacuous).

Decision-needed:

- [x] [Review][Patch][HIGH] **SignalR hub fails OPEN on unauthenticated `JoinTenant`.** `ChatBotProjectConversationHub.IsTenantAuthorized` returns `true` for any unauthenticated caller, the hub has no `[Authorize]`/`.RequireAuthorization()`, `UseAuthorization()` is never called, and the UI `HubConnection` carries no access token — so even in a JWT-on deployment an anonymous WebSocket reaches `JoinTenant`, `IsAuthenticated` is false, and the fail-open branch admits the connection to ANY `project-conversation:{tenantId}` group, leaking that tenant's existence + AI-activity *timing* (payload is metadata-only: the tenant id the caller supplied). `UseAuthentication()` only populates `User` from a present token — it does NOT reject anonymous requests. Broader than the story's disclosed "wire the UI token" caveat: the server-side `[Authorize]`/`UseAuthorization` gap remains even if the UI token is wired. Bounded today (notifications default-off; JWT not configured in dev/test), but the binding ADR mandates fail-closed on unauthorized/cross-tenant. [src/Hexalith.ChatBot.Server/Projections/ChatBotProjectConversationHub.cs:51-62; src/Hexalith.ChatBot.Server/Program.cs:83] — **Decision (2026-06-21, Jerome): HARDEN NOW (server + UI)** → add `[Authorize]`/`.RequireAuthorization()` + `app.UseAuthorization()`, gate the unauthenticated allow-through behind `IHostEnvironment.IsDevelopment()`, wire the UI `HubConnection` access token, and add a negative cross-tenant authz test (absorbs F11).

Patch:

- [x] [Review][Patch][MEDIUM] **Reconnect rejoins the group but never re-queries server state** (AC5 / ADR "rejoin AND re-query before rendering" violation). `connection.Reconnected += async _ => await JoinTenantAsync()` only rejoins; it never invokes `_onChanged`, and `ProjectConversationAiResponseReconnectAction` + its effect/reducer are built-but-never-dispatched dead code (the unit test exercises the effect in isolation → false confidence). A terminal transition arriving during the disconnect gap leaves the UI on stale in-flight state with an enabled Stop control until an unrelated action re-queries. Fix: have the `Reconnected` handler also trigger the existing re-query path. [src/Hexalith.ChatBot.UI/State/ProjectConversation/ProjectConversationStreamingSubscriber.cs:65]
- [x] [Review][Patch][LOW] **Benign duplicate/no-advance projection signal is surfaced as a "stale" error banner** instead of being silently deduped — contradicts the AC "duplicate-safe" requirement and the effect's own "dedups a redundant re-signal" comment. The tenant-wide at-least-once broadcast routinely delivers signals that don't advance the loaded conversation's own latest progress; `IsAcceptableNudge` rejects them → `StreamingErrorCode="ai-response-nudge-unsafe"` → "Response progress is stale; refreshing." Fix: distinguish a no-advance duplicate (silent no-op) from a true regression (banner). [src/Hexalith.ChatBot.UI/State/ProjectConversation/ProjectConversationReducers.cs:146,274-295]
- [x] [Review][Patch][LOW] **No behavioral test for hub cross-tenant authorization.** The `tenant-forbidden` branch and the `IsAuthenticated` allow-through have zero behavioral coverage — only source-substring scans in `AiResponseStreamingTransportAdrTests`. The E2E joins two anonymous same-process connections. Fix: add authenticated cross-tenant-join (expect `HubException` "tenant-forbidden") + authorized-join tests, reusing the existing `TestPrincipalStartupFilter`. (Couple to the F1 decision.) [tests/Hexalith.ChatBot.Server.Tests/Projections/ChatBotProjectConversationHubE2ETests.cs]

Defer:

- [x] [Review][Defer][LOW] **Cancel/Stop handler validates metadata only, not the target generation.** The cancel is routed to a fresh aggregate keyed by `CancellationId`, so it never confirms the target ResponseId/GenerationId exists / is in-flight / belongs to the same project/session, never checks `ExpectedSourceVersion` against real aggregate state, and the cancel write path isn't project-authorized (absent from `ParticipantAuthorizationStage.CanReadProject`). Bounded: tenant is server-bound (no cross-tenant), and the read side IS project-authorized (state-poisoning, not confidentiality). Carried LOW tied to the M2+ server-side generation lifecycle (matches the prior-pass deferral, story line ~526). [src/Hexalith.ChatBot.Server/Operations/GovernedOperationAggregate.cs:1648-1693] — deferred, pre-existing

Dismissed (11 false positives, verified): F2 (parasitic on F1; no new id leaked), F4 (`verifiedCancel` uses `.Any()` not ordering; single rendering row), F6 (row materializes synchronously BEFORE the advisory publish await), F7 (un-keyed announce flag but append-only monotonic versions make mis-fire unreachable — latent/defensive only), F8 (nudge synthesized from server-authoritative read, not wire data; first-nudge accept is correct), F9 (`ReduceLoad` nulls `StreamingErrorCode` on every load before `ReduceLoaded`), F12 (zero-width in-flight window = accurately-disclosed M2+ limitation, not a code defect), F13/F14 (trap rebuttals: producer wired + E2E real — both independently re-confirmed), F15 (AC2/AC3/AC1/AC6 safety floor verified intact), F16 (the 3 recurring FPs re-verified, no overturning evidence).

**Resolution (2026-06-21 review pass — all 4 patches applied; build 0/0; suites green):**

- **F1 (HIGH) FIXED — server fails closed; UI token seam wired.** `ChatBotProjectConversationHub` now injects `IConfiguration`; `IsTenantAuthorized` allows an unauthenticated caller ONLY when JWT bearer auth is not configured (`!ChatBotJwtAuthentication.IsConfigured(_configuration)`), so a JWT-on deployment fails closed (anonymous `JoinTenant` → `tenant-forbidden`). This is the load-bearing fix, gated at the SAME `IsConfigured` switch Program.cs already uses for `UseAuthentication`. **Implementation note / divergence from the decision preview:** I deliberately did NOT thread `app.UseAuthorization()` / `.RequireAuthorization()` middleware through the shared EventStore domain-service host — the config env-gate delivers the same fail-closed guarantee with far less blast radius on the shared pipeline and keeps the existing anonymous dev/test E2E green. `ChatBotHubEndpoint` gained an optional `AccessTokenProvider` forwarded to the UI `HubConnection`; **the UI has no token source today (confirmed by grep), so the provider is a deployment-supplied seam** — a JWT-on UI that doesn't supply it simply fails closed to typed reads (no live nudges), which is safe. [ChatBotProjectConversationHub.cs, ChatBotHubEndpoint.cs, ProjectConversationStreamingSubscriber.cs]
- **F5 (MEDIUM) FIXED.** The subscriber's `Reconnected` handler now rejoins the group AND invokes a new `onReconnected` callback; the workspace wires it to dispatch `ProjectConversationAiResponseReconnectAction` (which already re-queries via `LoadProjectConversationAction`), so the previously-dead reconnect re-query path is live and the disconnect-gap stale-state window is closed. [ProjectConversationStreamingSubscriber.cs, ChatBotProjectConversationWorkspace.razor]
- **F10 (LOW) FIXED.** `HandleProjectionSignalAsync` now silently dedups a benign no-advance re-signal (`nudge == _state.Value.LastAcceptedAiResponseNudge`) instead of dispatching a nudge the reducer rejects and surfaces as a spurious "stale" banner. [ProjectConversationEffects.cs]
- **F11 (LOW) FIXED.** Added three behavioral hub-authz tests — authenticated cross-tenant → `tenant-forbidden`, authenticated same-tenant → receives signal, unauthenticated + JWT-configured → fail-closed — reusing a principal-injecting startup filter. Plus a new effect dedup test (UI.Tests) and tightened architecture source-scans (reconnect re-query + dedup guard). [ChatBotProjectConversationHubE2ETests.cs, ProjectConversationEffectsTests.cs, AiResponseStreamingTransportAdrTests.cs]
- **F3 (LOW) DEFERRED** to `deferred-work.md` (cancel generation-validation; needs the M2+ server-side generation lifecycle). 11 false positives dismissed.

Verification: `dotnet build Hexalith.ChatBot.slnx --no-restore -m:1 /nr:false` — **0 errors, 0 warnings**. Compiled xUnit v3 runners: Server **1690/0** (+3 F11), UI.Tests **161/0** (+1 F10 dedup), UI.E2E **123/0** (browser path, ~25.8s), Architecture **63/0**. `git diff --check` — clean.
...
**2026-06-21 dev pass — closed the carried CRITICAL (AC1 producer + transport) and MEDIUM (effect/reducer divergence):**

- Two design decisions were taken with Jerome before any code (the prior 5 review passes had deliberately deferred them as needing an architecture call): (1) **producer = project the real governed execution lifecycle** (not a synthetic/staged generator); (2) **transport = reuse the EventStore signal-only `ProjectionChanged` channel via DAPR pub/sub relay** (not a bespoke ChatBot hub). Investigation also surfaced that ChatBot's `project-conversation` read model is an in-process ChatBot.Server projection (not an EventStore projection), so the reuse path uses a pub/sub relay rather than ChatBot self-hosting the hub — no EventStore/FrontComposer submodule source edits.
- **Producer** (`LowRiskAiOutcomeProjectionTranslator.FromStarted/FromCompleted`): the existing `PublishedAiOutcomeEvent → AiOutcomeEventView → ProjectConversationItemView.BuildAiResponseProgress()` pipe already threads the five `AiResponse*` progress fields; the translator now populates them — `executing` → non-terminal `rendering` (IsTerminal=false), `success` → `completed`, `failed` → `failed`, `pending-approval` → `unavailable` (all IsTerminal=true, metadata-only). Risky ask-AI stays proposal-first (no inline streamed response) per AC6.
- **Server transport seam**: `IProjectConversationChangePublisher` (no-op default; `DaprProjectConversationChangePublisher` reuses EventStore `ProjectionChangedNotification` + `NamingConventionEngine.GetProjectionChangedTopic`, fail-open) is invoked by `InMemoryProjectConversationProjectionStore.UpsertAiOutcomeEventAsync` only when a progress-bearing AI-outcome row is materialized (covers both producer progress and the existing cancellation `stopped` terminal). Enabled by `ChatBot:ProjectionChangeNotifications:Enabled=true` via `AddChatBotProjectionChangeNotifications` (off by default → deterministic tests). `ProjectConversationResponse.TenantContext` is now populated with the requester's own kebab tenant id (same-tenant metadata; nullable field, no OpenAPI change) so the UI can resolve the tenant for the group.
- **UI client**: `ProjectConversationStreamingSubscriber` (per-component, owns the FrontComposer `IProjectionSubscription` + `IProjectionChangeNotifierWithTenant` the UI already registers via `AddHexalithEventStore`) joins the tenant-scoped `project-conversation` group and, on a matching change signal, the workspace dispatches `ProjectConversationProjectionSignalReceivedAction`. The new effect `HandleProjectionSignalAsync` fails closed against the loaded conversation, synthesizes a forward-looking metadata-only nudge (last-rendered progress +1), and dispatches `ProjectConversationAiResponseNudgeReceivedAction` → typed re-query. The rich nudge/reducer/effect machinery is therefore dispatched at runtime.
- **MEDIUM fix**: `ProjectConversationEffects` now injects `IState<ProjectConversationState>`; `HandleAiResponseNudgeAsync` re-queries iff the reducer accepted this exact nudge (`ReferenceEquals(state.LastAcceptedAiResponseNudge, action.Nudge)`), so effect and reducer share the single `IsAcceptableNudge` gate. The 3 pinning tests were updated (effect now takes `IState`; a `FakeState` double; the nudge test drives the real reducer then the effect).
- **Honest scope caveats (documented in the resolved CRITICAL follow-up):** cross-process delivery is only verifiable on the live DAPR/Aspire + EventStore topology (in-sandbox verification is per-seam + the live flag); and with today's disabled/synchronous AI provider the in-flight `rendering` window is zero-width, so the visible Stop-enabled window materializes with a real async streaming provider (M2+). The LOW follow-up (cancellation in-flight verification) stays deliberately deferred — it needs a server-side generation-session aggregate that does not yet exist.

Verification (2026-06-21 dev pass):

- `dotnet build Hexalith.ChatBot.slnx --no-restore -m:1 /nr:false` — **0 errors, 0 warnings**.
- Compiled xUnit v3 runners (VSTest hits the sandbox socket limit): Contracts **484/0**, Client **36/0**, Server **1686/0** (+6 new: producer non-terminal/terminal progress + publish-seam), UI.Tests **160/0** (+1 net: signal-effect test; nudge test rewritten to drive reducer→effect), UI.E2E **123/0** (browser-optional lane; Chromium present — no E2E test changes this story), Architecture **63/0** (+3 new: producer / transport-reuse / client source-scan).
- `git diff --check` — clean.
- **Runtime prerequisite (live topology only; not exercisable in-sandbox):** end-to-end SignalR delivery needs the running EventStore host + DAPR pub/sub, `ChatBot:ProjectionChangeNotifications:Enabled=true` on the ChatBot.Server host, and an access-token provider configured for the `[Authorize]` `ProjectionChangedHub` in the UI's `AddHexalithEventStore`. In-sandbox coverage is per-seam (recording-publisher unit test, effect unit tests, source-scan) — the cross-process relay and a visible non-zero in-flight window (async streaming provider, M2+) are not reproducible here.

**2026-06-21 (later) — pivot to a ChatBot-owned hub after a live Tier-3 run (DAPR available):**

- Ran the live Tier-3 E2E (`TrivialGovernedCommandAspireE2eTests`, `HEXALITH_CHATBOT_TIER3=1`) against the real DAPR topology — green (22/0) with the dev-pass changes, confirming the durable spine is unaffected. The run ALSO exposed that the reuse relay could not deliver: the topology has only a `chatbot-pubsub` component (no `pubsub`), the publisher hardcoded `"pubsub"`, EventStore's controller subscribes on the const `"pubsub"`, and EventStore SignalR was off.
- Jerome chose to **pivot to a ChatBot-owned SignalR hub** (sidesteps all cross-host matching; directly verifiable in one host). Replaced `DaprProjectConversationChangePublisher` with `ChatBotProjectConversationHub` (tenant-grouped, fail-closed cross-tenant when authenticated) + `SignalRProjectConversationChangePublisher` (IHubContext broadcaster); `AddChatBotProjectionChangeNotifications` now `AddSignalR()` + registers the SignalR publisher, and Program.cs maps the hub when `ChatBot:ProjectionChangeNotifications:Enabled=true`. UI: `ProjectConversationStreamingSubscriber` is now a `HubConnection` to the ChatBot server hub (`ChatBotHubEndpoint` holds the base address); the workspace no longer injects the FrontComposer projection-subscription. The producer, the signal effect/synthesis, and the MEDIUM fix are unchanged.
- **New in-sandbox E2E proves delivery (no DAPR):** `ChatBotProjectConversationHubE2ETests` connects a REAL SignalR `HubConnection` over a `WebApplicationFactory` test server, joins the tenant group, POSTs a low-risk AI outcome (producer sets progress → store broadcasts), and asserts the client receives the tenant-scoped `ProjectConversationChanged` signal while a different tenant's group does NOT.
- Re-ran the live Tier-3 with the pivoted composition — still green (22/0).

Verification (2026-06-21 pivot):

- `dotnet build Hexalith.ChatBot.slnx --no-restore -m:1 /nr:false` — **0 errors, 0 warnings**.
- Compiled xUnit v3 runners: Contracts **484/0**, Client **36/0**, Server **1687/0** (+1: the hub broadcast→receive E2E), UI.Tests **160/0**, UI.E2E **123/0**, Architecture **63/0** (transport/client source-scans updated for the ChatBot hub). `git diff --check` — clean.
- Live Tier-3 DAPR E2E (`HEXALITH_CHATBOT_TIER3=1`): **22/0** before and after the pivot (durable spine unaffected; full topology composes + runs).
- Remaining caveat: the visible in-flight streaming window is still zero-width until a real async AI provider (M2+); and the UI hub `HubConnection` access token for a JWT-on deployment is a remaining prod-wiring item (the hub allows unauthenticated joins in dev/test and fail-closes cross-tenant when authenticated).

Verification (prior 2026-06-19/20 passes, retained for history):

- `./tests/Hexalith.ChatBot.Contracts.Tests/...` - 484 passed; `Client.Tests` - 36 passed; `Server.Tests` - 1680 passed; `UI.Tests` - 159 passed; `UI.E2E.Tests` - 123 passed; `Architecture.Tests` - 60 passed; `git diff --check` - passed.
...
Added/modified in the 2026-06-21 dev pass (AC1 producer + transport, MEDIUM fix):

- `src/Hexalith.ChatBot.Server/Projections/LowRiskAiOutcomeProjectionTranslator.cs` (producer: project executing/completed lifecycle as Ai response progress)
- `src/Hexalith.ChatBot.Server/Projections/IProjectConversationChangePublisher.cs` (new: publisher seam + no-op default)
- `src/Hexalith.ChatBot.Server/Projections/DaprProjectConversationChangePublisher.cs` (new: DAPR pub/sub relay reusing EventStore ProjectionChangedNotification)
- `src/Hexalith.ChatBot.Server/Projections/InMemoryProjectConversationProjectionStore.cs` (publish project-conversation change signal on AI-progress upsert)
- `src/Hexalith.ChatBot.Server/Gateway/ProjectConversationChangeNotificationServiceCollectionExtensions.cs` (new: AddChatBotProjectionChangeNotifications)
- `src/Hexalith.ChatBot.Server/Gateway/CommandGatewayServiceCollectionExtensions.cs` (register no-op change publisher default)
- `src/Hexalith.ChatBot.Server/Program.cs` (config-gated AddChatBotProjectionChangeNotifications)
- `src/Hexalith.ChatBot.Server/Queries/ChatBotReadQueryResultMapper.cs` (populate ProjectConversationResponse.TenantContext)
- `src/Hexalith.ChatBot.Server/Queries/ChatBotReadQueryHandlers.cs` (pass query.TenantId into the response mapper)
- `src/Hexalith.ChatBot.UI/State/ProjectConversation/ProjectConversationStreamingSubscriber.cs` (new: per-component projection-changed subscription, fail-open)
- `src/Hexalith.ChatBot.UI/State/ProjectConversation/ProjectConversationActions.cs` (ProjectConversationProjectionSignalReceivedAction)
- `src/Hexalith.ChatBot.UI/State/ProjectConversation/ProjectConversationEffects.cs` (inject IState; HandleProjectionSignalAsync; MEDIUM fix — re-query iff reducer accepted)
- `src/Hexalith.ChatBot.UI/Components/Governed/ChatBotProjectConversationWorkspace.razor` (own + drive the streaming subscriber)
- `tests/Hexalith.ChatBot.Server.Tests/Projections/AiOutcomeProjectionTests.cs` (producer progress + publish-seam tests)
- `tests/Hexalith.ChatBot.UI.Tests/ProjectConversationEffectsTests.cs` (IState/FakeState; reducer→effect agreement; signal-effect test)
- `tests/Hexalith.ChatBot.Architecture.Tests/AiResponseStreamingTransportAdrTests.cs` (producer / transport-reuse / client source-scan guards)

Pivot to ChatBot-owned hub (2026-06-21 later) — added/changed/removed:

- `src/Hexalith.ChatBot.Server/Projections/ChatBotProjectConversationHub.cs` (new: ChatBot-owned, tenant-grouped, metadata-only SignalR hub)
- `src/Hexalith.ChatBot.Server/Projections/SignalRProjectConversationChangePublisher.cs` (new: IHubContext broadcaster behind IProjectConversationChangePublisher)
- `src/Hexalith.ChatBot.Server/Projections/DaprProjectConversationChangePublisher.cs` (REMOVED — replaced by the SignalR publisher)
- `src/Hexalith.ChatBot.Server/Gateway/ProjectConversationChangeNotificationServiceCollectionExtensions.cs` (AddSignalR + register the SignalR publisher)
- `src/Hexalith.ChatBot.Server/Program.cs` (map ChatBotProjectConversationHub when enabled)
- `src/Hexalith.ChatBot.UI/State/ProjectConversation/ProjectConversationStreamingSubscriber.cs` (rewritten: HubConnection to the ChatBot hub instead of the FrontComposer projection-subscription)
- `src/Hexalith.ChatBot.UI/State/ProjectConversation/ChatBotHubEndpoint.cs` (new: holds the ChatBot base address for the hub connection)
- `src/Hexalith.ChatBot.UI/Program.cs` (register ChatBotHubEndpoint)
- `src/Hexalith.ChatBot.UI/Components/Governed/ChatBotProjectConversationWorkspace.razor` (inject ChatBotHubEndpoint; construct the hub-based subscriber)
- `tests/Hexalith.ChatBot.Server.Tests/Projections/ChatBotProjectConversationHubE2ETests.cs` (new: in-sandbox SignalR broadcast→receive E2E)

### Change Log

- 2026-06-21 (review pass — Opus 4.8 multi-agent code review, Jerome): Adversarial 3-layer review (Blind Hunter / Edge Case Hunter / Acceptance Auditor) + dedup + adversarial verify over the full story diff `0218429..HEAD`, focused on the never-reviewed transport pivot. AC1 producer+transport independently re-confirmed RESOLVED (producer runtime-wired at `AiOutcomeProjectionHandler.cs:35`; hub E2E non-vacuous). 20 raw → 16 deduped findings: 5 confirmed, 11 dismissed false positives (incl. the 3 recurring FPs + the zero-width-window M2+ caveat). Applied 4 patches — F1 (hub fail-open → env-gated fail-closed under JWT + UI token seam), F5 (reconnect now re-queries), F10 (benign duplicate signal no longer shows a stale banner), F11 (behavioral hub-authz tests). Deferred F3 (cancel generation-validation LOW → `deferred-work.md`). Build 0/0; Server 1690/0, UI 161/0, UI.E2E 123/0, Architecture 63/0; `git diff --check` clean. Status → done.
- 2026-06-21 (latest): Pivoted the transport to a ChatBot-owned SignalR hub (Jerome's call after the live finding). Replaced the DAPR pub/sub relay with `ChatBotProjectConversationHub` + `SignalRProjectConversationChangePublisher` (server) and a `HubConnection`-based `ProjectConversationStreamingSubscriber` (UI). Added an in-sandbox SignalR broadcast→receive E2E (`ChatBotProjectConversationHubE2ETests`) that PROVES tenant-scoped delivery without DAPR. Build 0/0; six suites green (Contracts 484, Client 36, Server 1687, UI 160, UI.E2E 123, Architecture 63); live Tier-3 22/0 before+after; git diff --check clean. CRITICAL resolved; Status set to review. Remaining: zero-width visible window until an async AI provider (M2+); UI hub access-token for JWT-on deployments; LOW (cancellation in-flight) still deferred.
- 2026-06-21 (later): Live Tier-3 DAPR run (`TrivialGovernedCommandAspireE2eTests` 22/0, full topology green with these changes) exposed that the reuse relay does NOT deliver end-to-end in the real ChatBot Aspire topology: the only pub/sub component is `chatbot-pubsub` (no `pubsub`), the publisher hardcodes `"pubsub"`, EventStore's notification controller subscribes on the compile-time const `"pubsub"`, and EventStore SignalR is off. CRITICAL re-opened; Status reverted to in-progress; awaiting a decision (finish reuse topology wiring vs pivot to ChatBot-owned hub vs defer live delivery to a follow-up). Producer + in-process publish seam + UI client machinery remain done and unit-verified; the live durable spine is unaffected.
- 2026-06-21: Dev pass (Jerome) — closed the carried CRITICAL (AC1 producer + nudge transport) and MEDIUM (nudge effect/reducer divergence). After a two-decision design step (producer = real governed execution lifecycle; transport = reuse EventStore signal-only ProjectionChanged channel via DAPR pub/sub relay), added the server-side AI-response-progress producer, the metadata-only project-conversation change publisher (config-gated, no-op default), TenantContext population, the UI projection-changed subscriber + signal effect, and the IState-gated nudge effect (re-query iff reducer accepted). Build 0/0; six suites green (Contracts 484, Client 36, Server 1686, UI 160, UI.E2E 123, Architecture 63); git diff --check clean. Cross-process relay + visible in-flight window remain live-topology/async-provider dependent (documented). LOW (cancellation in-flight verification) stays deliberately deferred. Status set to review.
