...
- **CAP-2**
  ...
  - **success:** Eligible read-only assistance remains scoped. Every AI-proposed state-changing, outbound, file-exposing, task-creating or assigning, tool-invoking, or on-behalf effect becomes a frozen mandatory-approval proposal, and the system executes it only through its allowlisted governed command.
- **CAP-3**
...
- **CAP-5**
  ...
  - **success:** Every committed mutation atomically includes its lifetime idempotency result, policy/approval references, and canonical envelope; sensitive attempts remain separate, and correction blocks stale AI context until every affected derived store acknowledges invalidation.
- **CAP-6**
...
- **CAP-7**
  ...
  - **success:** M0, M1, and M2 advance in strict order. M0 and M1 claims require their complete PRD gate and current A5, A6, and A13 evidence; M2 revalidates those gates and additionally requires A10 and A11.
...
- A9a qualification remains independent. Completing each stable A11 metric tuple is necessary but insufficient: A11 also requires the full PRD/addendum baseline and recalibration cycle, frozen supported-request mix, code-catalog drift proof, minimum metric families, dashboard state, alert routing, and exact-candidate burn evidence. Until the complete contract passes, affected rows and claims remain `unsupported`.
...
- A5, A6, and A13 block M0 and M1. M2 revalidates them and additionally requires A10 and A11. The PRD increment table remains the complete release gate, including all evidence, outcomes, counter-metrics, disable conditions, sequencing, and permitted claims.
