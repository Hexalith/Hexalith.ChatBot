...
| A10 | RPO ≤ 15 min and RTO ≤ 4 hr remain provisional MVP targets pending a successful hosted required live-recovery run with a retained artifact locator. Story 12.15 stood up live Aspire/DAPR fault injection for the EventStore outage, the topology-composed M365 subscription path, and the identity and Graph scoped outages, but publishes no measured figure: its local diagnostic bundle predates the current evidence-manifest contract and cannot be replayed through the shipped gate. The sandbox does not close the external-M365, durable-WORM, production-control, provider-scale, or measurable-ceiling residuals. | Architecture / DevOps | Ratify only after the scheduled or release workflow retains a complete passing artifact and its run/artifact locator is recorded in `.decision-log.md`. Note the lane's 180-second measurable recovery ceiling: a pass ratifies recovery within that ceiling, not RTO ≤ 4 hr, which needs a lane that can reach it or a separately evidenced pre-production drill. Un-ratify to provisional when the cited artifact ages past retention without a successor, when a later required run is not passing, or on a hosted `Unmeasurable`/structural-breach verdict; revisit when a named residual closes or retained evidence records a target deviation. |
