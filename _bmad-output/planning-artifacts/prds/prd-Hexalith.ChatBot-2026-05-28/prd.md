...
- Recovery: RPO ≤ 15 min and RTO ≤ 4 hr verified through a documented continuity drill; both targets remain provisional pending retained hosted Story 12.15 evidence (see A10).
...
- Recovery objectives verified through a continuity drill (RPO ≤ 15 min, RTO ≤ 4 hr — provisional per A10 pending a retained hosted run locator).
...
| A10 | RPO ≤ 15 min and RTO ≤ 4 hr remain provisional MVP targets. A complete passing hosted required live-recovery run with a retained artifact locator now exists — required release run [33066358280](https://github.com/Hexalith/Hexalith.ChatBot/actions/runs/33066358280), commit `17aa94d`, evidence run `01M11EYSDMP1ZF38B7KZA1A6FA` (2026-08-27), superseding the earlier `397507b` bundle. Both mandatory drills report `met` with measured figures (`eventstore-outage` RPO 0s / RTO 149.6s; `m365-subscription-failure` RPO 0s / RTO 60.5s, against the 900s/14400s targets), and all six scoped dependencies report `contained`. The lane's 180-second measurable-recovery-ceiling residual (`RV-MEASURABLE-CEILING`) still applies: a pass ratifies recovery within that ceiling, not RTO ≤ 4 hr, and the measured RPO is still a constant on the no-loss path rather than a citable non-constant loss-path measurement. The sandbox still does not close the external-M365, durable-WORM, production-control, or provider-scale residuals. | Architecture / DevOps | **A10 remains provisional** (Winston/Murat review, 2026-08-27) — the exit condition requires a lane whose claim limitations do not cover the target being ratified, and this lane's ceiling still covers RTO ≤ 4 hr. Ratifying the 4-hour RTO needs a lane that can reach it or a separately evidenced pre-production drill; ratifying the 15-minute RPO needs a citable non-constant loss-path measurement. Un-ratify to provisional when the cited artifact ages past retention without a successor, when a later required run is not passing, or on a hosted `Unmeasurable`/structural-breach verdict; revisit when a named residual closes or retained evidence records a target deviation. |
