...
public sealed class ProjectConversationE2ETests
{
    ...
    public async Task ProjectConversationShouldRenderClassificationDetectedIntentAiSummaryDefaultAndReviewHistory()
    {
        ...
        await using (harness)
        {
            ...
            AssertTextOrder(
                ...
                "Redaction state",
                "metadata_only");
            (await actionable.GetByRole(AriaRole.Button).CountAsync()).ShouldBe(0);
            ...
            (await aiItem.GetByRole(AriaRole.Button).CountAsync()).ShouldBe(0);
            ...
            AssertTextOrder(
                ...
                "Redacted",
                "redacted");
            IReadOnlyList<string> reviewHistoryLabels = await harness.Page
                .Locator(".chatbot-conversation-review-history[aria-label]")
                .EvaluateAllAsync<string[]>("nodes => nodes.map(node => node.getAttribute('aria-label') || '')");
            reviewHistoryLabels.Count.ShouldBe(reviewHistoryLabels.Distinct(StringComparer.Ordinal).Count());
            ...
            AssertTextOrder(
                ...
                "Why unavailable?",
                "Decision detail is unavailable on this surface.");
            ILocator redactionReason = redacted.Locator(".chatbot-conversation-classification__reason");
            (await redactionReason.GetAttributeAsync("tabindex")).ShouldBe("0");
            await redactionReason.FocusAsync();
            (await redactionReason.EvaluateAsync<bool>("element => document.activeElement === element")).ShouldBeTrue();
    ...
    public async Task TaskIntentReviewPanelShouldExposeReviewConversionAndDispositionWorkflow()
    {
        ...
        await using (harness)
        {
            ...
            ILocator review = harness.Page.Locator("[data-chatbot-task-intent-id='task-intent:review-001']");
            ...
            ILocator transitions = harness.Page.GetByRole(AriaRole.Region, new() { NameString = "Available transitions" });
            await WaitForVisibleAsync(transitions);
            AssertTextOrder(
                await transitions.InnerTextAsync(),
            ...
            ILocator auditHistory = harness.Page.GetByRole(AriaRole.Region, new() { NameString = "Audit history" });
            await WaitForVisibleAsync(auditHistory);
            await WaitForVisibleAsync(auditHistory.GetByText("audit-transition-001", new() { Exact = true }));
    ...
    public async Task OutboundApprovalGateShouldPauseSendUntilApprovalAndRetainMetadataOnlyDecisions()
    {
        ...
        await using (harness)
        {
            ...
            await request.GetByRole(AriaRole.Button, new() { NameRegex = new("^Approve$") }).ClickAsync();
    ...
    public async Task ProjectConversationLowRiskAiExecutionRowsShouldRenderPolicyContextAndProviderFailureMetadata()
    {
        ...
        await using (harness)
        {
            ...
            await WaitForVisibleAsync(succeededItem.GetByText("AI summary provenance", new() { Exact = true }));
    ...
    public async Task ProjectConversationGovernedComposerShouldExposeSubmissionStatesAndSuppressTextEntryShortcuts()
    {
        ...
        await using (harness)
        {
            ...
            ILocator messageMode = activeComposer.GetByRole(AriaRole.Button, new() { NameString = "Message" });
            ILocator askAiMode = activeComposer.GetByRole(AriaRole.Button, new() { NameString = "Ask AI" });
            await WaitForVisibleAsync(messageMode);
            await WaitForVisibleAsync(askAiMode);
            (await messageMode.GetAttributeAsync("aria-pressed")).ShouldBe("true");
            (await askAiMode.GetAttributeAsync("aria-pressed")).ShouldBe("false");
            await askAiMode.ClickAsync();
            (await messageMode.GetAttributeAsync("aria-pressed")).ShouldBe("false");
            (await askAiMode.GetAttributeAsync("aria-pressed")).ShouldBe("true");
            await messageMode.ClickAsync();
            (await messageMode.GetAttributeAsync("aria-pressed")).ShouldBe("true");
            (await askAiMode.GetAttributeAsync("aria-pressed")).ShouldBe("false");
            ...
            (await textbox.InnerTextAsync()).ShouldBe("a");
            (await activeComposer.Locator("button, textarea, label").CountAsync()).ShouldBe(0);
            (await activeComposer.Locator("fluent-button").CountAsync()).ShouldBe(3);
            (await activeComposer.Locator("fluent-label").CountAsync()).ShouldBe(1);
            (await activeComposer.Locator("fluent-text-area").CountAsync()).ShouldBe(1);
            ...
            (await validation.Locator("button, textarea, label").CountAsync()).ShouldBe(0);
            ...
            (await pending.Locator("button, textarea, label").CountAsync()).ShouldBe(0);
            ...
            (await unauthorized.Locator("button, textarea, label").CountAsync()).ShouldBe(0);
            ...
            (await degraded.Locator("button, textarea, label").CountAsync()).ShouldBe(0);
        ...
        => """
            ...
            <section class="chatbot-governed-composer composer-action-entry"
              ...
              <div class="chatbot-governed-composer__modes" role="group" aria-label="Governed composer">
                <fluent-button role="button" tabindex="0" type="button" aria-pressed="true" onclick="setComposerMode('message')">Message</fluent-button>
                <fluent-button role="button" tabindex="0" type="button" aria-pressed="false" onclick="setComposerMode('ask-ai')">Ask AI</fluent-button>
              </div>
              <fluent-label id="project-conversation-composer-label" class="chatbot-labelled-row" for="project-conversation-composer-input">Message or governed request</fluent-label>
              <fluent-text-area id="project-conversation-composer-input"
                                role="textbox"
                                tabindex="0"
                                contenteditable="true"
                                aria-labelledby="project-conversation-composer-label"
                                aria-describedby="project-conversation-composer-help"
                                placeholder="Write a governed project update or request."
                                onkeydown="event.stopPropagation();"></fluent-text-area>
              ...
              <div class="chatbot-governed-composer__actions">
                <fluent-button role="button" tabindex="0" type="submit" appearance="primary">Submit</fluent-button>
            ...
            <section class="chatbot-governed-composer composer-action-entry" aria-label="Governed composer validation error">
              ...
              <fluent-label id="project-conversation-composer-validation-label" class="chatbot-labelled-row" for="project-conversation-composer-validation-input">Message or governed request</fluent-label>
              <fluent-text-area id="project-conversation-composer-validation-input"
                                role="textbox"
                                tabindex="0"
                                contenteditable="true"
                                aria-labelledby="project-conversation-composer-validation-label"
                                aria-invalid="true"
                                aria-describedby="project-conversation-composer-error"></fluent-text-area>
              <fluent-button role="button" tabindex="0" type="submit" appearance="primary">Submit</fluent-button>
            </section>
            <section class="chatbot-governed-composer composer-action-entry" aria-label="Governed composer command accepted">
              ...
              <fluent-label id="project-conversation-composer-pending-label" class="chatbot-labelled-row" for="project-conversation-composer-pending-input">Message or governed request</fluent-label>
              <fluent-text-area id="project-conversation-composer-pending-input"
                                role="textbox"
                                tabindex="0"
                                contenteditable="true"
                                aria-labelledby="project-conversation-composer-pending-label"></fluent-text-area>
            </section>
            <section class="chatbot-governed-composer composer-action-entry" aria-label="Governed composer unauthorized">
              ...
              <fluent-label id="project-conversation-composer-unauthorized-label" class="chatbot-labelled-row" for="project-conversation-composer-unauthorized-input">Message or governed request</fluent-label>
              <fluent-text-area id="project-conversation-composer-unauthorized-input"
                                role="textbox"
                                tabindex="-1"
                                aria-labelledby="project-conversation-composer-unauthorized-label"
                                aria-disabled="true"></fluent-text-area>
              <fluent-button role="button" tabindex="-1" type="submit" appearance="primary" aria-disabled="true">Submit</fluent-button>
            </section>
            <section class="chatbot-governed-composer composer-action-entry" aria-label="Governed composer dependency degraded">
              ...
              <fluent-label id="project-conversation-composer-degraded-label" class="chatbot-labelled-row" for="project-conversation-composer-degraded-input">Message or governed request</fluent-label>
              <fluent-text-area id="project-conversation-composer-degraded-input"
                                role="textbox"
                                tabindex="-1"
                                aria-labelledby="project-conversation-composer-degraded-label"
                                aria-disabled="true"></fluent-text-area>
              <fluent-button role="button" tabindex="-1" type="submit" appearance="primary" aria-disabled="true">Submit</fluent-button>
            </section>
            ...
            <script>
              function setComposerMode(mode) {
                const composer = document.querySelector('[aria-label="Governed composer active"]');
                const modes = composer.querySelectorAll('fluent-button[aria-pressed]');
                modes[0].setAttribute('aria-pressed', mode === 'message' ? 'true' : 'false');
                modes[1].setAttribute('aria-pressed', mode === 'ask-ai' ? 'true' : 'false');
              }
                ...
                <li class="chatbot-conversation-stream__entry">
                  <article class="chatbot-participant-conversation-item"
                    ...
                    <header class="chatbot-participant-conversation-item__header">
                      ...
                      <span class="chatbot-actor-badge" aria-label="External party actor: Unresolved participant">Unresolved participant <button type="button">Why unavailable?</button></span>
                ...
                <li class="chatbot-conversation-stream__entry">
                  <article class="chatbot-participant-conversation-item"
                    ...
                    <header class="chatbot-participant-conversation-item__header">
                      ...
                      <span class="chatbot-participant-conversation-item__status">Resolved</span>
                      <span class="chatbot-actor-badge" aria-label="External party actor: Restricted participant">Restricted participant <button type="button">Why unavailable?</button></span>
                ...
                <li class="chatbot-conversation-stream__entry">
                  <article class="chatbot-email-conversation-item" data-chatbot-conversation-item-kind="EmailDerived" data-chatbot-conversation-item-id="01HZXMAILBOX000000000000012" tabindex="0" aria-label="Mailbox item: Classification redacted, Associated">
                    ...
                    <section class="chatbot-conversation-classification" aria-label="Classification for item 01HZXMAILBOX000000000000012" data-chatbot-classification="informational">
                      ...
                      <p class="chatbot-conversation-classification__reason" tabindex="0"><strong>Why unavailable?</strong> Decision detail is unavailable on this surface.</p>
                    </section>
                  </article>
                </li>
              </ol>
        ...
        => """
            <article class="chatbot-approval-conversation-item"
              ...
              <div class="chatbot-approval-conversation-item__actions" aria-label="Approval decision">
                <fluent-button role="button" tabindex="0" aria-disabled="true" aria-describedby="approval-approve-reason" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();this.click();}" onclick="const status=document.getElementById('approval-decision-status'); status.setAttribute('role','alert'); status.setAttribute('aria-live','assertive'); status.textContent='Evidence expired';">
                  Approved
                </fluent-button>
                <fluent-button role="button" tabindex="0" onclick="const status=document.getElementById('approval-decision-status'); status.setAttribute('role','status'); status.setAttribute('aria-live','polite'); status.textContent='Rejected';">
                  Rejected
                </fluent-button>
                <fluent-button role="button" tabindex="0" onclick="const status=document.getElementById('approval-decision-status'); status.setAttribute('role','status'); status.setAttribute('aria-live','polite'); status.textContent='Requested revision';">
                  Requested revision
                </fluent-button>
                <fluent-button role="button" tabindex="0" onclick="const status=document.getElementById('approval-decision-status'); status.setAttribute('role','status'); status.setAttribute('aria-live','polite'); status.textContent='Cancelled';">
                  Cancelled
                </fluent-button>
              </div>
    ...
    private static string BuildFreshApprovalDecisionSurfaceBody()
        => """
            ...
            <article class="chatbot-approval-conversation-item"
              ...
              <div class="chatbot-approval-conversation-item__actions" aria-label="Approval decision">
                <fluent-button role="button" tabindex="0" aria-disabled="false" onclick="window.__submitFreshApproval();">
                  Approved
                </fluent-button>
                <fluent-button role="button" tabindex="0" onclick="const status=document.getElementById('fresh-approval-decision-status'); status.setAttribute('role','status'); status.setAttribute('aria-live','polite'); status.textContent='Rejected';">
                  Rejected
                </fluent-button>
                <fluent-button role="button" tabindex="0" onclick="const status=document.getElementById('fresh-approval-decision-status'); status.setAttribute('role','status'); status.setAttribute('aria-live','polite'); status.textContent='Requested revision';">
                  Requested revision
                </fluent-button>
                <fluent-button role="button" tabindex="0" onclick="const status=document.getElementById('fresh-approval-decision-status'); status.setAttribute('role','status'); status.setAttribute('aria-live','polite'); status.textContent='Cancelled';">
                  Cancelled
                </fluent-button>
              </div>
            ...
            <ol class="chatbot-conversation-stream__list" aria-label="Project conversation stream">
              <li class="chatbot-conversation-stream__entry">
                <article class="chatbot-approval-conversation-item" data-chatbot-conversation-item-kind="ApprovalEvent" data-chatbot-conversation-item-id="approval:outbound-approval-001:request:63" tabindex="0" aria-label="Approval event, Outbound approval requested, Pending, 2026-06-02 09:00:00Z">
                  ...
                  <div class="chatbot-approval-conversation-item__actions" aria-label="Outbound approval decision">
                    <fluent-button role="button" tabindex="0" onclick="window.__approveOutbound();">Approve</fluent-button>
                    <fluent-button role="button" tabindex="0" onclick="const status=document.getElementById('outbound-approval-status'); status.setAttribute('role','status'); status.setAttribute('aria-live','polite'); status.textContent='Rejected';">Reject</fluent-button>
                    <fluent-button role="button" tabindex="0" onclick="const status=document.getElementById('outbound-approval-status'); status.setAttribute('role','status'); status.setAttribute('aria-live','polite'); status.textContent='Requested revision';">Request revision</fluent-button>
                    <fluent-button role="button" tabindex="0" onclick="const status=document.getElementById('outbound-approval-status'); status.setAttribute('role','status'); status.setAttribute('aria-live','polite'); status.textContent='Cancelled';">Cancel</fluent-button>
                    <fluent-button id="outbound-send-button" role="button" tabindex="0" aria-disabled="true" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();this.click();}" onclick="window.__executeOutbound();">Execute approved send</fluent-button>
                  </div>
              ...
              <li class="chatbot-conversation-stream__entry">
                <article class="chatbot-approval-conversation-item" data-chatbot-conversation-item-kind="ApprovalEvent" data-chatbot-conversation-item-id="approval:outbound-approval-005:request:67" tabindex="0" aria-label="Approval event, Outbound approval requested, Blocked, 2026-06-02 09:08:00Z">
                  ...
                  <div class="chatbot-approval-conversation-item__actions" aria-label="Outbound approval decision"><fluent-button role="button" tabindex="0" aria-disabled="true" aria-describedby="outbound-approval-blocked-reason">Approve</fluent-button></div>
    ...
    private static void AssertClassificationCoverageWithoutBrowser()
    {
        ...
        fixture.ShouldNotContain("<button", Case.Insensitive);
        AssertTextOrder(
            fixture,
            "data-chatbot-ai-content=\"source-evidence\"",
            "<details class=\"chatbot-ai-outcome-conversation-item__generated\"",
    ...
    private static void AssertGovernedComposerWithoutBrowser()
    {
        ...
        composer.ShouldContain("<FluentButton");
        composer.ShouldContain("<FluentLabel");
        composer.ShouldContain("<FluentTextArea");
        ...
        composer.ShouldNotContain("<button", Case.Sensitive);
        composer.ShouldNotContain("<textarea", Case.Sensitive);
        composer.ShouldNotContain("<label", Case.Sensitive);
        composer.ShouldNotContain("chatbot-button", Case.Sensitive);
        ...
        fixture.ShouldContain("<fluent-button");
        fixture.ShouldContain("<fluent-label");
        fixture.ShouldContain("<fluent-text-area");
        fixture.ShouldNotContain("<button", Case.Sensitive);
        fixture.ShouldNotContain("<textarea", Case.Sensitive);
        fixture.ShouldNotContain("<label", Case.Sensitive);
        fixture.ShouldNotContain("chatbot-button", Case.Sensitive);
