...
using SubmitOutboundChannelQuarantine = Hexalith.ChatBot.Contracts.Commands.SubmitOutboundChannelQuarantine;
...
public sealed class OutboundChannelQuarantineAuthorizationTests
{
    ...
    public async Task QuarantineCommandsShouldRejectInvalidMetadataOnlyPayloads()
    {
        ...
        foreach (SubmitOutboundChannelQuarantine invalid in new[]
                 {
                     QuarantineSubmit() with { SourceVersion = -1 },
                     QuarantineSubmit() with { SchemaVersion = "outbound-channel-control-schema.custom" },
                     QuarantineSubmit() with { ReasonCode = "unsafe reason" },
                     QuarantineSubmit() with { NewState = OutboundChannelControlState.Active },
                     QuarantineSubmit() with { OldState = OutboundChannelControlState.Quarantined },
                 })
        {
            ChatBotAuthorizationResult denied = await stage.AuthorizeAsync(
                Submission(invalid),
    ...
    private static SubmitOutboundChannelQuarantine QuarantineSubmit()
        => new(
    ...
    private static ApproveOutboundChannelQuarantine QuarantineApproval()
        => new(
            "outbound-channel-quarantine-001",
            OutboundChannel,
            "outbound-channel-policy-violation",
