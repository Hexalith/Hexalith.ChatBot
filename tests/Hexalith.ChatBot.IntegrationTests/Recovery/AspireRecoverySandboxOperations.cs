...
internal sealed class AspireRecoverySandboxOperations : IRecoverySandboxOperations
{
    ...
    /// cross-tenant/fault-probe checks that use this window complete, so a full <see cref="_absenceConfirmationWindow"/>
    ...
    private static readonly TimeSpan DefaultPresenceConfirmationWindow = TimeSpan.FromMinutes(1);
    ...
    private readonly RecoverySandboxOperationsTestSeam? _testSeam;
    private readonly TimeSpan _pollInterval;
    private readonly TimeSpan _absenceConfirmationWindow;
    private readonly TimeSpan _presenceConfirmationWindow;
    /// <summary>
    ...
    private EventStoreRecoveryCleanupState _eventStoreCleanupState = new();
    private SubscriptionRecoveryCleanupState _subscriptionCleanupState = new();
        ...
        EventStoreDurableStateProbe durableState)
    {
        ...
        _presenceConfirmationWindow = DefaultPresenceConfirmationWindow;
    }

    /// <summary>Initializes infrastructure-free concrete producer and cleanup operations for always-run tests.</summary>
    ...
    /// <param name="presenceConfirmationWindow">The projection-presence observation window.</param>
    internal AspireRecoverySandboxOperations(
        RecoverySandboxOperationsTestSeam testSeam,
        IReadModelStore readModelStore,
        IReadModelConditionalEraser readModelEraser,
        EventStoreDurableStateProbe durableState,
        TimeSpan pollInterval,
        TimeSpan absenceConfirmationWindow,
        TimeSpan presenceConfirmationWindow)
    {
        ...
        ArgumentOutOfRangeException.ThrowIfLessThanOrEqual(presenceConfirmationWindow, TimeSpan.Zero);
        ...
        _presenceConfirmationWindow = presenceConfirmationWindow;
    }

    /// <summary>Gets the currently active EventStore cleanup generation for regression assertions.</summary>
    internal EventStoreRecoveryCleanupState ActiveEventStoreCleanupState => _eventStoreCleanupState;

    /// <summary>Gets the currently active subscription cleanup generation for regression assertions.</summary>
    internal SubscriptionRecoveryCleanupState ActiveSubscriptionCleanupState => _subscriptionCleanupState;
        ...
        CancellationToken cancellationToken)
    {
        ...
        string userAccessToken = await AcquireUserAccessTokenAsync(cancellationToken).ConfigureAwait(false);
        string controlAccessToken = await AcquireControlAccessTokenAsync(cancellationToken).ConfigureAwait(false);
        string operationRef = ChatBotTaskId.New().Value;
        _eventStoreCleanupState.CheckpointNoteRefs.Clear();
        _eventStoreCleanupState.CheckpointCommittedAtUtc.Clear();
        _eventStoreCleanupState.CheckpointCorrelationId = correlationId;
        for (int index = 0; index < CommittedCheckpointRecordCount; index++)
        {
            ...
            _eventStoreCleanupState.CheckpointNoteRefs.Add(noteRef);
            ...
            _eventStoreCleanupState.CheckpointCommittedAtUtc.Add(UtcNow);
        }
        ...
        _eventStoreCleanupState.ControlTenantNoteRef = controlNoteRef;
        ...
        await _durableState.WaitForGovernedNoteAsync(
            ...
            controlNoteRef,
            cancellationToken).ConfigureAwait(false);
        return new RecoveryOperationCheckpoint(_eventStoreCleanupState.CheckpointNoteRefs.Count, _eventStoreCleanupState.CheckpointCommittedAtUtc.Max(), operationRef);
    }
        ...
        CancellationToken cancellationToken)
    {
        string userAccessToken = await AcquireUserAccessTokenAsync(cancellationToken).ConfigureAwait(false);
        string noteRef = GovernedNoteId.New().Value;
        _eventStoreCleanupState.FaultProbeNoteRef = noteRef;
    ...
    public async ValueTask<DateTimeOffset> WaitForEventStoreRecoveryAsync(CancellationToken cancellationToken)
    {
        await Application.ResourceNotifications
            .WaitForResourceHealthyAsync(EventStoreResource.Name, cancellationToken)
            .ConfigureAwait(false);
        ...
        while (timer.Elapsed < ListenerReadinessBudget)
        {
            ...
            try
            {
                using HttpResponseMessage response = await EventStoreClient.GetAsync("/health", cancellationToken).ConfigureAwait(false);
    ...
    private async Task<ListenerGap> DescribeListenerGapAsync()
    {
        ...
        string retained = EventStoreClient.BaseAddress?.ToString() ?? "none";
        string resolved;
        try
        {
            resolved = Application.GetEndpoint(EventStoreResource.Name, "http").ToString();
        }
        ...
        try
        {
            using HttpClient fresh = Application.CreateHttpClient(EventStoreResource.Name, "http");
    ...
    public async ValueTask<RecoveryEventStoreEndState> ReadEventStoreEndStateAsync(
        ...
        CancellationToken cancellationToken)
    {
        if (_eventStoreCleanupState.CheckpointNoteRefs.Count == 0)
        {
            throw new InvalidOperationException("No committed recovery checkpoint exists.");
        }

        if (string.IsNullOrWhiteSpace(_eventStoreCleanupState.ControlTenantNoteRef))
        {
        ...
        foreach (string noteRef in _eventStoreCleanupState.CheckpointNoteRefs)
        {
        ...
        if (!string.IsNullOrWhiteSpace(_eventStoreCleanupState.FaultProbeNoteRef))
        {
            Task<bool> durableFaultAbsent = _durableState.RemainsGovernedNoteAbsentAsync(
                RecoveryValidationTopology.StorageTenantRef,
                _eventStoreCleanupState.FaultProbeNoteRef,
                _absenceConfirmationWindow,
                cancellationToken);
            // Positive control first: prove the channel can read a view this run OBSERVED the server write.
            // Using _eventStoreCleanupState.CheckpointNoteRefs[0] unconditionally asserted a note the loop above explicitly permits to be
        ...
        List<Task<bool>> crossTenantAbsenceChecks = _eventStoreCleanupState.CheckpointNoteRefs
        ...
        crossTenantAbsenceChecks.Add(_durableState.RemainsGovernedNoteAbsentAsync(
            RecoveryValidationTopology.StorageTenantRef,
            _eventStoreCleanupState.ControlTenantNoteRef,
            _absenceConfirmationWindow,
            cancellationToken));
        ...
        using HttpResponseMessage foreignRead = await GetAuthorizedAsync(
            $"/api/v1/governed-operations/{_eventStoreCleanupState.ControlTenantNoteRef}",
    ...
    {
        EventStoreRecoveryCleanupState cleanupState = EventStoreRecoveryCleanupState.DetachAndReset(
            ref _eventStoreCleanupState);
        if (!await IsEventStoreEndpointAvailableAsync(cancellationToken).ConfigureAwait(false))
        {
            throw new InvalidOperationException("EventStore was not available during continuity cleanup verification.");
        }

        if (!cleanupState.HasOwnedState)
        {
            return true;
        }

        if (string.IsNullOrWhiteSpace(cleanupState.CheckpointCorrelationId))
        {
        ...
        foreach (string noteRef in cleanupState.CheckpointNoteRefs)
        {
            try
            {
        ...
        if (!string.IsNullOrWhiteSpace(cleanupState.ControlTenantNoteRef))
        {
            try
            {
                if (!await IsGovernedOperationProjectionPresentAsync(
                    RecoveryValidationTopology.ControlTenantRef,
                    cleanupState.ControlTenantNoteRef,
                    cancellationToken).ConfigureAwait(false))
                {
                    complete = false;
        ...
        if (!string.IsNullOrWhiteSpace(cleanupState.FaultProbeNoteRef))
        {
            try
            {
                ...
                if (!await RemainsGovernedOperationProjectionAbsentAsync(
                    RecoveryValidationTopology.StorageTenantRef,
                    cleanupState.FaultProbeNoteRef,
                    cancellationToken).ConfigureAwait(false))
                {
                    complete = false;
        ...
        {
            await EraseReadModelAsync(
                GovernedOperationView.KeyFor(RecoveryValidationTopology.ControlTenantRef, cleanupState.ControlTenantNoteRef),
                cancellationToken).ConfigureAwait(false);
        }
        if (!string.IsNullOrWhiteSpace(cleanupState.FaultProbeNoteRef))
        {
            await EraseReadModelAsync(
                GovernedOperationView.KeyFor(RecoveryValidationTopology.StorageTenantRef, cleanupState.FaultProbeNoteRef),
                cancellationToken).ConfigureAwait(false);
        }

        try
        {
            List<Task<bool>> absenceChecks = cleanupState.CheckpointNoteRefs
        ...
        if (!complete)
        {
            string diagnostic = FormatIncompleteCleanupDiagnostic(incompleteChecks);
            await WriteCleanupDiagnosticAsync(diagnostic, cancellationToken).ConfigureAwait(false);
        }

        return complete;
    }

    /// <summary>Formats stable metadata-only EventStore cleanup diagnostics.</summary>
    /// <param name="incompleteChecks">The stable verification codes that failed.</param>
    /// <returns>The metadata-only diagnostic line.</returns>
    internal static string FormatIncompleteCleanupDiagnostic(IEnumerable<string> incompleteChecks)
    {
        ArgumentNullException.ThrowIfNull(incompleteChecks);
        HashSet<string> allowed = new(StringComparer.Ordinal)
        {
            "checkpoint-note-not-present",
            "checkpoint-note-presence-faulted",
            "control-note-not-present",
            "control-note-presence-faulted",
            "fault-probe-projection-present",
            "fault-probe-absence-faulted",
            "erased-projection-still-present",
            "post-erase-absence-faulted",
        };
        List<string> distinct = [];
        foreach (string code in incompleteChecks)
        {
            if (!allowed.Contains(code))
            {
                throw new InvalidOperationException("Continuity cleanup received an unsupported diagnostic code.");
            }

            if (!distinct.Contains(code, StringComparer.Ordinal))
            {
                distinct.Add(code);
            }
        }

        return $"RECOVERY_CLEANUP_INCOMPLETE checks={string.Join(',', distinct)}";
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        (string? intakeId, string? candidateRef) = CaptureResponseIdentities(
            root,
            _subscriptionCleanupState.StorageIntakeRefs,
            requireEquality: true,
            "Subscription checkpoint returned an invalid cleanup identity.");
        ...
        if (intakeId is null || candidateRef is null)
        {
            throw new InvalidOperationException(
                "Subscription committed-before-outage checkpoint did not return both contract identities.");
        }
        ...
        await _durableState.WaitForMailboxIntakeAsync(
            ...
            intakeId,
            cancellationToken).ConfigureAwait(false);
        _subscriptionCleanupState.SubscriptionCheckpointIntakeRef = intakeId;
        ...
        CancellationToken cancellationToken)
    {
        ...
        JsonElement root = process.RootElement;
        (string? intakeRef, string? candidateRef) = CaptureResponseIdentities(
            root,
            _subscriptionCleanupState.StorageIntakeRefs,
            requireEquality: true,
            "The controlled-loss retained bound returned an invalid cleanup identity.");
        foreach (string identity in EnumeratePresent(intakeRef, candidateRef))
        {
            _subscriptionCleanupState.ControlTenantAbsenceRefs.Add(identity);
        }

        if (!root.GetProperty("submitted").GetBoolean() || intakeRef is null || candidateRef is null)
        {
        ...
        if (preFault)
        {
            _subscriptionCleanupState.ControlledPreFaultIntakeRef = intakeRef;
        }
        else
        {
            _subscriptionCleanupState.ControlledPostRecoveryIntakeRef = intakeRef;
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        (string? intakeRef, string? candidateRef) = CaptureResponseIdentities(
            ...
            _subscriptionCleanupState.StorageIntakeRefs,
            requireEquality: true,
            "The controlled-loss rejection returned an invalid cleanup identity.");
        foreach (string identity in EnumeratePresent(intakeRef, candidateRef))
        {
            _subscriptionCleanupState.ControlTenantAbsenceRefs.Add(identity);
        }

        if (candidateRef is not null)
        {
            _subscriptionCleanupState.StorageDurableAbsenceRefs.Add(candidateRef);
        }
        ...
        if (!rejected || candidateRef is null ||
        ...
        ProjectConversationSourceEmailView? affectedAfterFault = await _readModels
            .GetSourceEmailAsync(_subscriptionCleanupState.AffectedTenantSentinel!.TenantId, _subscriptionCleanupState.AffectedTenantSentinel.IntakeId, cancellationToken)
            .ConfigureAwait(false);
        ProjectConversationSourceEmailView? controlAfterFault = await _readModels
            .GetSourceEmailAsync(_subscriptionCleanupState.ControlTenantSentinel!.TenantId, _subscriptionCleanupState.ControlTenantSentinel.IntakeId, cancellationToken)
            .ConfigureAwait(false);
        _subscriptionCleanupState.ControlledFaultLeftStateUnchanged = Equals(affectedAfterFault, _subscriptionCleanupState.AffectedTenantSentinel) &&
            Equals(controlAfterFault, _subscriptionCleanupState.ControlTenantSentinel);
        _subscriptionCleanupState.ControlledRejectedCandidateRef = candidateRef;
    ...
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(tenantRef);
        if (_subscriptionCleanupState.ControlledPreFaultIntakeRef is null || _subscriptionCleanupState.ControlledRejectedCandidateRef is null ||
            _subscriptionCleanupState.ControlledPostRecoveryIntakeRef is null || _subscriptionCleanupState.AffectedTenantSentinel is null ||
            _subscriptionCleanupState.ControlTenantSentinel is null)
        {
        ...
        Task<bool> candidateAggregateAbsent = _durableState.RemainsAbsentAsync(
            RecoveryValidationTopology.StorageTenantRef,
            _subscriptionCleanupState.ControlledRejectedCandidateRef,
            _absenceConfirmationWindow,
            cancellationToken);
        Task<bool> candidateReadModelsAbsent = _intakeReadModels.RemainsAbsentAsync(
            RecoveryValidationTopology.StorageTenantRef,
            _subscriptionCleanupState.ControlledRejectedCandidateRef,
            _absenceConfirmationWindow,
            cancellationToken);
        Task<bool> controlPreAggregateAbsent = _durableState.RemainsAbsentAsync(
            RecoveryValidationTopology.ControlTenantRef,
            _subscriptionCleanupState.ControlledPreFaultIntakeRef,
            _absenceConfirmationWindow,
            cancellationToken);
        Task<bool> controlPreReadModelsAbsent = _intakeReadModels.RemainsAbsentAsync(
            RecoveryValidationTopology.ControlTenantRef,
            _subscriptionCleanupState.ControlledPreFaultIntakeRef,
            _absenceConfirmationWindow,
            cancellationToken);
        Task<bool> controlCandidateAggregateAbsent = _durableState.RemainsAbsentAsync(
            ...
            _absenceConfirmationWindow,
            cancellationToken);
        Task<bool> controlCandidateReadModelsAbsent = _intakeReadModels.RemainsAbsentAsync(
            RecoveryValidationTopology.ControlTenantRef,
            _subscriptionCleanupState.ControlledRejectedCandidateRef,
            _absenceConfirmationWindow,
            cancellationToken);
        Task<bool> controlPostAggregateAbsent = _durableState.RemainsAbsentAsync(
            RecoveryValidationTopology.ControlTenantRef,
            _subscriptionCleanupState.ControlledPostRecoveryIntakeRef,
            _absenceConfirmationWindow,
            cancellationToken);
        Task<bool> controlPostReadModelsAbsent = _intakeReadModels.RemainsAbsentAsync(
            RecoveryValidationTopology.ControlTenantRef,
            _subscriptionCleanupState.ControlledPostRecoveryIntakeRef,
            _absenceConfirmationWindow,
            cancellationToken);
        ...
        ProjectConversationSourceEmailView? affectedSentinel = await _readModels.GetSourceEmailAsync(
            _subscriptionCleanupState.AffectedTenantSentinel.TenantId,
            _subscriptionCleanupState.AffectedTenantSentinel.IntakeId,
            cancellationToken).ConfigureAwait(false);
        ProjectConversationSourceEmailView? controlSentinel = await _readModels.GetSourceEmailAsync(
            _subscriptionCleanupState.ControlTenantSentinel.TenantId,
            _subscriptionCleanupState.ControlTenantSentinel.IntakeId,
            cancellationToken).ConfigureAwait(false);
        bool sentinelsUnchanged = Equals(affectedSentinel, _subscriptionCleanupState.AffectedTenantSentinel) &&
            Equals(controlSentinel, _subscriptionCleanupState.ControlTenantSentinel);
        return EvaluateControlledLossSafety(
            ...
            sentinelsUnchanged,
            _subscriptionCleanupState.ControlledFaultLeftStateUnchanged);
    }
    ...
    public async ValueTask ExpireSubscriptionAsync(string tenantRef, CancellationToken cancellationToken)
    {
        ...
        _subscriptionCleanupState.AffectedTenantSentinel = SubscriptionSentinel(
            RecoveryValidationTopology.StorageTenantRef,
            "recovery-subscription-affected-sentinel");
        _subscriptionCleanupState.ControlTenantSentinel = SubscriptionSentinel(
            RecoveryValidationTopology.ControlTenantRef,
            "recovery-subscription-control-sentinel");
        await _readModels.UpsertSourceEmailAsync(_subscriptionCleanupState.AffectedTenantSentinel, cancellationToken).ConfigureAwait(false);
        await _readModels.UpsertSourceEmailAsync(_subscriptionCleanupState.ControlTenantSentinel, cancellationToken).ConfigureAwait(false);
        ...
        CancellationToken cancellationToken)
    {
        ...
        JsonElement root = response.RootElement;
        _ = CaptureResponseIdentities(
            root,
            _subscriptionCleanupState.StorageIntakeRefs,
            requireEquality: true,
            "The subscription fault response returned an invalid cleanup identity.");
        if (root.GetProperty("submitted").GetBoolean() ||
        ...
        ProjectConversationSourceEmailView? controlAfterFault = await _readModels
            .GetSourceEmailAsync(
                ...
                cancellationToken)
            .ConfigureAwait(false);
        _subscriptionCleanupState.SubscriptionFaultLeftStateUnchanged = Equals(affectedAfterFault, _subscriptionCleanupState.AffectedTenantSentinel) &&
            Equals(controlAfterFault, _subscriptionCleanupState.ControlTenantSentinel);
        ...
        CancellationToken cancellationToken)
    {
        ...
        (string? reconciledIntakeRef, string? reconciledCandidateRef) = CaptureResponseIdentities(
            processRoot,
            _subscriptionCleanupState.StorageIntakeRefs,
            requireEquality: true,
            "The restored Worker returned an invalid cleanup identity.");
        if (!processRoot.GetProperty("submitted").GetBoolean())
        {
            string kind = processRoot.GetProperty("kind").GetString() ?? "unknown";
            string reasonCode = processRoot.GetProperty("reasonCode").GetString() ?? "unknown";
            throw new InvalidOperationException(
                $"The restored Worker path did not reconcile the seeded notification (kind={kind}, reason={reasonCode}).");
        }

        if (reconciledIntakeRef is null || reconciledCandidateRef is null)
        {
            throw new InvalidOperationException("The restored Worker did not return both contract identities.");
        }

        _subscriptionCleanupState.ReconciledIntakeRef = reconciledIntakeRef;
        ...
        (string? duplicateIntakeRef, string? duplicateCandidateRef) = CaptureResponseIdentities(
            duplicateRoot,
            _subscriptionCleanupState.StorageIntakeRefs,
            requireEquality: true,
            "The duplicate-notification probe returned an invalid cleanup identity.");
        foreach (string identity in EnumeratePresent(duplicateIntakeRef, duplicateCandidateRef))
        {
            _subscriptionCleanupState.StorageDurableAbsenceRefs.Add(identity);
        }
        ...
        _subscriptionCleanupState.DuplicateProbeIntakeRef = duplicateIntakeRef;
        ...
        bool isolated = Equals(affectedSentinel, _subscriptionCleanupState.AffectedTenantSentinel) &&
            Equals(controlSentinel, _subscriptionCleanupState.ControlTenantSentinel) &&
            isolationOutcomes[1] &&
            isolationOutcomes[2];
        ...
        bool noDuplicateProjection = !string.Equals(duplicateIntakeRef, reconciledIntakeRef, StringComparison.Ordinal) &&
            isolationOutcomes[0];
        return new RecoverySubscriptionEndState(
            ...
            UnauthorizedMutationAbsent: _subscriptionCleanupState.SubscriptionFaultLeftStateUnchanged);
    }

    /// <inheritdoc />
    public async ValueTask<bool> CleanupSubscriptionScenarioAsync(string tenantRef, CancellationToken cancellationToken)
    {
        SubscriptionRecoveryCleanupState cleanupState = SubscriptionRecoveryCleanupState.DetachAndReset(
            ref _subscriptionCleanupState);
        List<(string Tenant, string Intake)> cleanupTargets = [];
        AddIntakeTarget(cleanupTargets, cleanupState.AffectedTenantSentinel);
        AddIntakeTarget(cleanupTargets, cleanupState.ControlTenantSentinel);
        foreach (string intakeRef in cleanupState.StorageIntakeRefs)
        {
            AddIntakeTarget(cleanupTargets, RecoveryValidationTopology.StorageTenantRef, intakeRef);
        }

        foreach (string intakeRef in cleanupState.ControlTenantAbsenceRefs)
        {
            AddIntakeTarget(cleanupTargets, RecoveryValidationTopology.ControlTenantRef, intakeRef);
        }
    ...
    /// </summary>
    private static (string? IntakeRef, string? CandidateRef) CaptureResponseIdentities(
        JsonElement root,
        ...
        CancellationToken cancellationToken)
    {
        ...
        return await commands
            .ExecuteCommandAsync(EventStoreResource, command, cancellationToken)
            .ConfigureAwait(false);
    }

    private async ValueTask<bool> IsEventStoreEndpointAvailableAsync(CancellationToken cancellationToken)
    {
        if (_testSeam?.IsEventStoreEndpointAvailableAsync is { } testAvailability)
        {
    ...
    private async Task SubmitUntilAcceptedAsync(
        ...
        CancellationToken cancellationToken)
    {
        if (_testSeam?.SubmitUntilAcceptedAsync is { } testSubmit)
        {
            await testSubmit(
                noteRef,
                commandRef,
        ...
        CancellationToken cancellationToken)
    {
        ...
        Stopwatch timer = Stopwatch.StartNew();
        while (timer.Elapsed < _presenceConfirmationWindow)
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", accessToken);
        request.Headers.Add("X-Correlation-Id", correlationId);
        return await ChatBotClient.SendAsync(request, cancellationToken).ConfigureAwait(false);
    }
    ...
    {
        if (_testSeam?.SendSandboxControlAsync is { } testControl)
        {
            return await testControl(
                tenantRef,
                action,
                includeBearer,
                cancellationToken,
                notificationPhase,
                scenarioLane).ConfigureAwait(false);
        }
        ...
        if (includeBearer)
        {
            string mailboxAccessToken = await AcquireMailboxAccessTokenAsync(cancellationToken).ConfigureAwait(false);
    ...
    private async ValueTask<string> AcquireUserAccessTokenAsync(CancellationToken cancellationToken)
    {
        if (_testSeam?.AcquireUserAccessTokenAsync is { } testAcquire)
        {
            return await testAcquire(cancellationToken).ConfigureAwait(false);
        }

        return await RecoveryAccessTokenProvider.AcquireAsync(Application, cancellationToken).ConfigureAwait(false);
    }
    ...
    private async ValueTask<string> AcquireMailboxAccessTokenAsync(CancellationToken cancellationToken)
        => await RecoveryAccessTokenProvider
            .AcquireMailboxAsync(Application, MailboxClientSecret, cancellationToken)
            .ConfigureAwait(false);

    private async ValueTask WriteCleanupDiagnosticAsync(string diagnostic, CancellationToken cancellationToken)
    {
        if (_testSeam?.WriteDiagnosticAsync is { } testWriter)
        {
            await testWriter(diagnostic, cancellationToken).ConfigureAwait(false);
            return;
        }

        await Console.Error.WriteLineAsync(diagnostic.AsMemory(), cancellationToken).ConfigureAwait(false);
    }

    private DistributedApplication Application => _application ??
        throw new InvalidOperationException("This recovery operation requires the live Aspire application.");

    private IResource EventStoreResource => _eventStoreResource ??
        throw new InvalidOperationException("This recovery operation requires the live EventStore resource.");

    private HttpClient ChatBotClient => _chatBotClient ??
        throw new InvalidOperationException("This recovery operation requires the live ChatBot client.");

    private HttpClient EventStoreClient => _eventStoreClient ??
        throw new InvalidOperationException("This recovery operation requires the live EventStore client.");

    private HttpClient SandboxClient => _sandboxClient ??
        throw new InvalidOperationException("This recovery operation requires the live recovery-sandbox client.");

    private string MailboxClientSecret => _mailboxClientSecret ??
        throw new InvalidOperationException("This recovery operation requires the mailbox client secret.");

    private string ControllerSecret => _controllerSecret ??
        throw new InvalidOperationException("This recovery operation requires the recovery controller secret.");
