    ...
    string appHostVersion) : IRecoveryValidationEvidenceSink
{
        ...
        int coverage)
    {
        ...
        RecoveryValidationEvidenceManifest manifest = new()
        {
            ...
            ProjectionPreRebuildSchemaVersion = (report as ProjectionRebuildReport)?.PreRebuildSchemaVersion,
            ProjectionRebuiltSchemaVersion = (report as ProjectionRebuildReport)?.RebuiltSchemaVersion,
            ProjectionPreRebuildDigests = (report as ProjectionRebuildReport)?.PreRebuildDigests,
            ProjectionRebuiltDigests = (report as ProjectionRebuildReport)?.RebuiltDigests,
            ProjectionSourceResourceCount = (report as ProjectionRebuildReport)?.SourceResourcesCompared ?? 0,
            ProjectionGovernedResourceCount = (report as ProjectionRebuildReport)?.GovernedResourcesCompared ?? 0,
            ProjectionWormRecordCount = (report as ProjectionRebuildReport)?.WormRecordsReplayed ?? 0,
            ProjectionWormOperationCount = (report as ProjectionRebuildReport)?.WormOperationsReplayed ?? 0,
            ProjectionFingerprintAlgorithmVersion = (report as ProjectionRebuildReport)?.FingerprintAlgorithmVersion,
            ProjectionPreRebuildFingerprint = (report as ProjectionRebuildReport)?.PreRebuildFingerprint,
            ProjectionRebuiltFingerprint = (report as ProjectionRebuildReport)?.RebuiltFingerprint,
